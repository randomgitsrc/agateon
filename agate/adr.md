...
> 实现注记：本增补属平台接入细节的决策记录，非协议语义定义。

**决策**：平台适配载体（agent md / preset / skill）**只做「指向 + 平台差异」**，协议内容**单一来源**在 `orchestrator-template.md`。**机械守护是关键词级第一道网，不追求语义级判据**；**防线归属 = 人工评审**（改协议或适配层须过 protocol-alignment-review），与 `LIMITATIONS.md` 局限 3 的通用口径一致。
...
- 故本 ADR 明确：**守护的定位是"让最常见形态在 CI 变红"，不是"保证不漂"**；上限登记为 `DEBT0049`（已按本决策关闭）。
