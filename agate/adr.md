...
> 实现注记：本增补**不引入新决策**，只把平台适配层的既有安排**就地记录**并指向权威口径——
> 「机械兜底不覆盖语义、防线归人工评审」早已写在 `LIMITATIONS.md` 局限 3（此处仅落到适配层这一具体面）。
> 属平台接入细节，非协议语义定义。

**记录**：平台适配载体（agent md / preset / skill）**只做「指向 + 平台差异」**，协议内容**单一来源**在 `orchestrator-template.md`。**机械守护是关键词级第一道网**；**防线归属 = 人工评审**（改协议或适配层须过 protocol-alignment-review）——沿用 `LIMITATIONS.md` 局限 3 的既有口径，非本次新定。
