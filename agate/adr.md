...
- 某 ADR 的前提被后续变更证伪时，须在该 ADR 处**就地标注「已过时 + 被什么取代」**（指向取代它的 ADR 或变更），**不删除**——保留决策史，让后来者看清"当时为何如此、后来为何不再如此"。
- 复审时机：① 每次新增 ADR 时，顺带复核相关旧 ADR 的前提是否仍成立；② P7 一致性检查阶段，核对本任务的变更是否证伪了某条既有决策。
- 复审是人工判断与就地标注，不设自动失效或阻断流程的机制。
...
已接受（用户 2026-09-21 确认）。**本增补修正下文「决策」节的项目级路径为可选形态**——只读该节的读者请一并读本增补。
...
> `[HUMAN_CONFIRMED: 2026-09-21]` 用户确认「默认全局注册」决策（原话：「安装时直接给他们全局都配置好不就可以了」；本增补按 A7 要求提请人工裁决后确认）。采纳理由：四平台均原生支持全局配置，且 `orchestrator-template.md` 跨项目内容一致、`{agate_root}` 按 cwd 运行时解析——全局注册与项目钉版不冲突，逐项目重复注册纯属重复劳动且易漏。
...
### 增补（2026-09-21b）：平台适配层「去漂移」的防线归属

> 实现注记：本增补属平台接入细节的记录，非协议语义定义。**通用口径**（机械兜底不覆盖语义、
> 防线归人工评审）早于本次写在 `LIMITATIONS.md` 局限 3——本增补只把该口径**落到平台适配层这一面**，
> 并登记其上限；**上限的处置选项与触发条件以 `DEBT0049` 为准**（本增补不复述，避免两处权威）。

**记录**：平台适配载体（agent md / preset / skill）**只做「指向 + 平台差异」**，协议内容**单一来源**在 `orchestrator-template.md`。**机械守护是关键词级第一道网**；**防线归属 = 人工评审**（改协议或适配层须过 protocol-alignment-review）。守护上限与备选方案见 `DEBT0049`。

**为什么不做语义级机械判据**（含 2026-09-21 实测依据）：

- 现有判据（`agate/tests/conftest.py` 的 `PROTOCOL_RULE_MARKERS` 等）拦的是**逐字复述 / 照抄表头**——这是**实际发生过**的漂移形态（DSH persona 曾抄会话开始步骤，模板改路径后副本未跟）；守护对改动前文件**会变红**（回归有效，已实测）。
- 但**同义改写即可绕过**（审查用探针实证）。把它升级为语义判据的代价高于收益，且会给出**虚假的安全感**——评审误以为"有守护就不会漂"。
- 故本 ADR 明确：**守护的定位是"让最常见形态在 CI 变红"，不是"保证不漂"**；上限登记为 `DEBT0049`（**观察项，保持 open**：若再发生一次改写式复制导致的实际漂移，则启动备选的结构性判据）。

**边界**：漂移检测（`agate-summary.py`）只覆盖**全局**形态的接入产物；`--scope project` 的项目内产物不检测（随项目走、目录各异，其一致性由项目自身版本控制保证）。
