...
A: 是——复制模式不自动同步，需**重跑 `python3 ~/.agate/scripts/agate-setup.py`** 刷新。软链模式无此问题。**该情形会被自动检出**：`agate-summary.py` 对已接入平台的产物做检测（覆盖 `agate-setup.py` 支持的全部平台），复制形态比对内容，副本已旧时报「已过期」并给出上述修复命令。
