...
> ↑ 上述 `max_depth=1` /「无法再派发」记于 subagent workflows 默认启用之前；时效更新见上方 `## Codex` 章「子代理派发（`spawn_agent`）」小节（`multi_agent` flag 实测 stable/true、`spawn_agent` 单层已实测可用、嵌套深度经 P6 V7 复核实测 `depth=2` 可用——`max_depth=1` 结论已推翻）。
