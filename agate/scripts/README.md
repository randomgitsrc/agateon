...
| 脚本 | 用途 |
|------|------|
| `agate-install.py` | 安装/卸载 agate 版本 + 环境探测：无参 / `latest` = 装 latest 指针（最新发布版，幂等）；`vX.Y.Z` = 装指定版本（`repo/` 单克隆作对象库，由 `agate_package` 从 tag 构建**只含本体**的版本目录，不再是 git worktree / 整仓检出；幂等）；`--adopt vX.Y.Z` = 纳管已就位的版本目录（写 `latest`/`current` 指针 + 同步根 `scripts/`，不碰 git，portable 与离线安装的收尾步骤）；`--uninstall vX` = 移除版本目录 + 指针清理（含项目引用保护扫描；仅旧整仓形态且 `repo/` 存在时才 `git worktree remove`）；`--check` = 环境探测（python3/pyyaml/git/bash，分平台修复指引），`--check --portable` = opt-in，必需项仅 python3 + pyyaml，缺 git/bash 只提示；`~/.agate` 是软链时 fail-closed 并打印迁移三步 |
| `agate-setup.py` | **平台接入命令**（2026-09-21）：自动探测已装平台（Claude Code / OpenCode / DSH / Codex）→ 注册 orchestrator 身份（**默认全局**：`~/.claude/agents/` · `~/.config/opencode/agents/` · `~/.dsh/skills/` + DSH profile 的 `cordis.patch.yml` 声明块（`preset-agate`，2026-09-24 起；DSH ≥0.1.7-alpha.1 不再读取旧的 `.agent-presets/` 目录） · `~/.agents/skills/agate-protocol/`；`--scope project` 落项目内目录）→ 装 git hook。幂等；既有非本工具文件先备份；Windows 无符号链接权限时退化复制。参数：`--platform a,b` / `--scope all\|global\|project`（默认 all = 注册全局身份 + 装 hook；global = 只注册全局身份不装 hook；project = 注册到项目内目录 + 装 hook）/ `--dry-run`。替代 SETUP.md「步骤 2」的逐平台手工 `ln -sf`/`cp` 步骤 ；另有 `--list`（看已装）/ `--uninstall [--scope global\|project] [--all-projects] [--dry-run] [--purge]`（卸载：按事实验证归属、保留用户数据、还原用户原 hook）|
...
| `agate-pack-offline.py` | 外网离线打包器：`vX.Y.Z [--platform linux-x86_64\|windows-x86_64] [--outdir DIR] [--repo DIR] [--ref REF] [--include-python] [--include-pillow]` → 平台标签 bundle + manifest.json（sha256 checksum + `files` 清单 + `source_ref`）。bundle 顶层 = `agate/`（本体，由 `agate_package` 从 git 对象构建）+ 登记根文件 + `wheels/` + `manifest.json`，不再有 `agate/agate/scripts` 双层嵌套；`--ref` 可取预发布 tag 内容；`--outdir` 已存在时拒绝覆盖（不清空、不删除既有内容）|
...
| `install-offline.py` | 内网离线安装器：`<bundle_dir> [--dest-root DIR] [--skip-python] [--skip-pillow]`。软链基址守卫 → manifest 平台核对（不匹配拒绝）→ bundle 布局校验（旧格式双层嵌套 bundle 拒绝并要求用新版打包器重打、软链/特殊文件拒绝、磁盘文件与 manifest `files` 对账）→ checksum 校验（不匹配拒绝）→ 只把 manifest 已登记的本体包拷进带标记的临时工作目录 → `pip install --no-index --find-links wheels/` → 换位为 `vX.Y.Z/`（已存在则旧目录先入带标记备份容器，即替换语义）→ 同目录兄弟 `agate-install.py --adopt` 写指针；adopt 失败则还原版本目录与指针，成功才删本次创建的备份容器。启动时只清扫带有效标记的过期临时目录，绝不按名字模式删除其他内容；不再写 `.installed-version` / `.agate-root` |
