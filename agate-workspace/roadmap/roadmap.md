...
| RM-AG0063 | **MVWU（最小可验证工作单元）协议落地**：把既有 batch 实践升格为协议原语（四不变量 Boundary/Verification/Provenance/Composability + 最小契约 `batches[].tests_filter` + `P4-evidence/{batch}.log` + 字段预算 ≤5）——设计定稿见 `docs/design-notes/design-mvwu-protocol.md`；阶段 1 零协议内核改动（旁路声明 + 证据 + 不阻断 check）。**⚠ 范围收窄（2026-09-19，TAG0036 P8）：本条 `done` 仅指设计文档【阶段 1】交付（工具 + 采集能力 + 文档成文；Q1/Q3 样本待真实多批任务出现，未答）；阶段 2（批级 gate，触内核）与阶段 3（`E_pipeline` 弹性度量）未开工——前置为「Q1∧Q2 ≥90% 试点数据 + 提交粒度用户裁决」，均未满足，不因本条 done 而视为已解决。** | done | 编排模型演进分析（`docs/design-notes/design-orchestration-evolution-analysis.md` v6.2）+ 三轮外部评审；2026-09-16 用户确认入 roadmap | **TAG0036** | 2026-09-16 | 2026-09-19 |
...
| RM-AG0066 | **安装与多版本模型统一**（Release 发布 + 本体安装包 + 三路径结构统一 + 离线解析失效修复）：① 引入 GitHub Release（tag push 自动建，notes 取 CHANGELOG 段）+ 本体 tarball asset（**portable**，解压即用，无需 git）② 统一在线/离线/legacy 三条安装路径的**目录结构契约** ③ 修 **P0 离线安装解析失效**（pack 用 worktree 检出整仓 → `bundle/agate/agate/` 多一层嵌套 → `_protocol_root` 探测不到 scripts）④ ~~本体目录名可扩展~~（2026-09-19 删除——与 `design-rename-execution.md` §8.1「`agate/` 永久保留，不议 `core/`」冲突）⑤ 在线安装**只装本体**（现状 43M vs 本体 4.1M，冗余 91%）；**保留**「装任意历史 tag」能力（即 `repo/` 保留） | done | 用户 2026-09-19 提出（安装机制审计后续） | TAG0037 | 2026-09-19 | 2026-09-20 |
...
| RM-AG0069 | **`install.sh` 重跑时更新已存在的 `repo/`**（升级自举缺口，RM-AG0066/TAG0037 P2 §13 backlog 建议）：`install.sh` 遇 `repo/` 已存在时不更新，≤v0.72 用户用旧安装器升级到 v0.73.0 会装出旧整仓形态（仅文档说明"两步升级"，见 `agate/UPGRADING.md` `### v0.73.0` 第 7 条）。拟做：`repo/` 已存在时先 `git pull --ff-only`（失败降级为 WARNING、不阻断、不改写本地历史），使重跑 `install.sh` 即可获得最新安装器；相关联 backlog：`_sync_root_scripts` 条件同步（钉老版本时不回退根 `scripts/`，eng m-5）。**触 SELF-GATE（`install.sh`）**；P2 主 Agent 裁定本任务不做（超出 P0 范围锁定） | backlog | TAG0037 P8（P2 [SCOPE+] 裁定 / P2 §13 backlog 建议，2026-09-20） | — | 2026-09-20 | 2026-09-20 |
| RM-AG0070 | **`agate-install.py latest` 从 Release asset 取包**（RM-AG0066 拟议模型第 2 项，TAG0037 P1 已声明不属本任务）：v0.73.0 起 Release 已含本体 tarball + 离线包 + `SHA256SUMS`，但在线升级仍是 `repo/` 克隆 + `agate_package` 从 tag 构建；本条让 `latest` 改为下载 Release 本体 asset 并核对 `SHA256SUMS`（`SHA256SUMS` 只防损坏、不认证发布者），**保留** `repo/` 以支持"装任意历史 tag"（用户 2026-09-19 已定取舍 #2）。需先定：无网络/Release 缺失时回退到 `repo/` 路径的语义、与 `--adopt` 的关系 | backlog | TAG0037 P8（RM-AG0066 拟议模型第 2 项，2026-09-20） | — | 2026-09-20 | 2026-09-20 |
## 状态标识
...
**MVWU（最小可验证工作单元）协议落地（2026-09-16，源自编排模型演进分析）

- **问题**：`dispatch_plan.batches[]` 目前只声明**执行切分**（id + complexity）；批的实际产出、验证证据、边界**无结构化声明**——批级测试证据散落在 commit message 散文里（实测 26 条），**没有任何 gate 消费它**，L0 验证是自律而非机制。
- **核心洞察（实测支撑）**：四不变量**几乎已有机械锚点**，不是待建的新子系统——I1 = 批级 commit 文件集（git 原生）；I2 = 批自带测试文件（`test_tag0034_p4c.py`）+ 批自带证据（`P4-implementation-P4c.md`）；I3 = `check-p6-provenance.py` 七道审计（成熟）；I4 = 任务级拆分实践。**MVWU 的工作量在于"把事实写成声明"。**
- **最小改动集**：① `batches[].tests_filter`（`P2-design.md` frontmatter 可选键，`_gate_p2_dispatch_plan` 不拒绝未知键故为零内核改动）② `P4-evidence/{batch}.log` 结构化落点 + 一条**不阻断**的独立 check。
- **阶段划分**：阶段 1 零内核改动（旁路声明 + 证据 + check）→ 阶段 2 批级 gate（**触及内核**）→ 阶段 3 `E_pipeline` 弹性度量。**在阶段 1 试点数据出来前不得进入阶段 2。**
- **已知约束（实测）**：I1 可归属率仅 **13%**（16 个多批任务中 2 个为逐批 commit）；且"批是否逐批 commit"**无稳定命名约定**（`TAG0016` 用 `-P4a`、`TAG0034` 用中文"P4a 批"、多数不标记）——合并提交形态的批 boundary 必然判 `UNKNOWN`。
- **试点判据（Q1–Q3）**：Q1 `tests_filter` 是否在多数批稳定可执行；Q2 边界是否多数批可机械比对；Q3 `1 batch = 1 MVWU` 是否成立。**Q1∧Q2 稳定成立（≥90%）才进阶段 2。**
- **验证口径**：`check-mvwu.py` 六项检查产出四态 verdict（`PASS/FAIL/EXPECTED_RED/UNKNOWN`，**`UNKNOWN ≠ PASS`**），**只观测不改 `.state.yaml`**；未声明 MVWU 时既有 34 任务 P2 gate 全绿（A4 回归）。
- **归属**：独立任务 **TAG0036**（2026-09-16 立项，仅阶段 1；设计文档 §7 已备试点范围）；不依赖任何时长归因（该归因未定，见分析 §3.2）。
- **完成判据（2026-09-16 修订，TAG0036 P0 明确）**：交付「**工具 + 采集能力 + 已有结论**」，**不含**「Q1/Q3 已答」——① `tests_filter` 键可用 ② `P4-evidence` 落点生效 ③ `check-mvwu.py` 可用 ④ **Q2 结论在案**（13%，已有实测）⑤ **Q1/Q3 采集流程就绪**（样本出现时自动产出行）。**Q1/Q3 需真实多批任务样本**（当前 34 任务已全完成）——**不主动造样本**（避免"自己验证自己"偏差），**不用历史回填**（事后推断测不出"当时是否可执行"，属自欺）。
- **前置澄清（2026-09-16 立项时核定）**：**无硬前置**（设计文档 §6：阶段 1 前置 = 无）。两个**软依赖不阻塞**：① 活动遥测——本任务的 Q1 数据与遥测互补，非依赖 ② TAG0035（fail-open）——阶段 1 **不挂 gate**，不受影响。**阶段 2 的前置** = ① 本任务 Q1∧Q2 ≥90% 的试点数据 ② TAG0035 完成（阶段 2 要改 `check-gate.py`，与 TAG0035 同文件）。
- **⚠ SELF-GATE 澄清**：本任务虽为「零**协议内核**改动」（不改 `.state.yaml`/gate/hook/审计链），但 `check-mvwu.py` 落在 `agate/scripts/*.py` → **仍命中 SELF-GATE 触发面**，须走完整 SELF-GATE 链。
...
**gate 对未知阶段 fail-open（2026-09-16，编排模型演进分析 §5.3 实测发现）

- **问题**：`agate/scripts/check-gate.py` 的 phase 分派：
  ```python
  ...
  ```
  **新增阶段若忘记注册 gate 函数，gate 会静默"通过"而非报错**——与 agateon 其他处的 fail-closed 取向（如 DSH preset 挂载失败拒绝建会话）相反。
- **为什么现在修**：与 DAG / MVWU 议题**无关**，是分析过程中独立查出的**确定缺陷**；且 `phases.yaml` 已支持自定义阶段扩展（`custom-role.md` 有既有范式），扩展时踩中的概率不低。
- **修复方向**：① 未知阶段改为 **exit 1**（fail-closed）② 新增 regression 测试（构造未知 phase → 断言 exit 1 且 stderr 含阶段名）③ 核对 `check-state-transition.py` / `pre-commit-gate.py` 是否有同类 fail-open。
- **验证口径**：`python3 agate/scripts/check-gate.py P99 <task_dir>` → **exit 1**（当前为 exit 2）；全量 pytest 无退化；consistency 0 ERROR。
