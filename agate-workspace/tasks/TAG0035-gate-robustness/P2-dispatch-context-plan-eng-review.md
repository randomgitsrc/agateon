...
> 本次是 P2-design.md 首次评审（非重试）。你与 architect 是不同 agent（信息隔离），独立判断，不预设方案一定合格。单评审角色（domains=[backend]，risk_level=medium，非 full ceremony，无 NEED_CONFIRM）——只你一人评审，无需组长汇总，产出直接写 P2-review.md。

### 目标

独立评审 `P2-design.md`（TAG0035 gate 健壮性批，4 子批实现方案），判定 `status: approved / rejected`。

### 约束

评审重点（按本方案实际决策点定制）：

1. **候选方案是否真实对比、非稻草人**：候选方案 2（集中式共享工具函数 + 状态字段驱动放宽 + 硬编码路径白名单）是否是真实可行但确有缺陷的方案，而不是刻意写弱的陪衬。核对候选 2 的三处"真实缺点"陈述（WARNING 语义无法被共享函数覆盖 / `retries[P4]` 非空即放行削弱拦截力 / 硬编码列表扩展性差）是否站得住脚——你可以假设候选 2 真的被采纳，推演一下是否会出问题。
2. **子批 B 的根因判断是否正确**：设计对 `pre-commit-gate.py` 的分析称"`_phase_num()` 的 `None` 分支在当前两处调用点（L291、L576）目前实际不可达，真正问题在 `_P_OUTPUT_RE` 上游过滤层"。这是一个具体的代码事实断言，建议你自己读一遍 `pre-commit-gate.py` 的 `_P_OUTPUT_RE` 定义与两处调用点，核实这个断言是否成立——如果断言错误，整个子批 B 第 3 点的设计方向就站不住脚。
3. **子批 C 的红灯边界是否真的成立**：设计称 `_gate_p4_has_prior_code_commit` 只在存在"含代码 diff 的历史 commit"时返回 True，"纯文档、无代码历史、非回退"场景必然返回 False。核对这个函数设计（commit message 含 `wf(<task_id>-P4` 标签 → 逐个 `git diff-tree` 检查非 md/yaml 文件）是否真的不会被"纯文档"场景意外命中——尤其要考虑：如果同一个 task_id 曾经有过其它阶段（如 P1/P2）的 commit，commit message 是否可能意外匹配 `--grep="wf(<task_id>-P4"` 这个前缀（如 `wf(TAG0035-P40)` 这类边界情况，虽然不太可能但需要确认 `--fixed-strings` 是否真的能防止子串误配）。
4. **子批 D 的 BDD-14 红灯用例是否真的还能拦住**：设计称"裸文件名场景（如 `p6-acceptance.md` 前面没有路径字符）时，token == exact，不含 phase-cards/，走 hits.append"。核对 `re.finditer(r"[\w./\-]*" + re.escape(exact), low)` 这个正则在真实的 dispatch-context 自述文本里（如 markdown 反引号包裹 `` `P6-acceptance.md` ``、或裸写 "参考 P6-acceptance.md"）是否确实不会意外匹配到无关的 `phase-cards/` 前缀，导致真实自述被误豁免。
5. **风险表是否覆盖了「实现完成的标志」列出的验收口径**：核对第 9 节 Definition of Done 是否与 P1 的 14 条 BDD 逐条对应，未遗漏（尤其 BDD-2 的 ci-gate-backstop.py 验证点、BDD-6 的差集扫描逻辑）。
6. **gate_commands 声明纪律**：核对 P5 三条 pytest 分片用 `&&` 拼接是否违反卡片"`--strict` 反模式"节的禁令——设计正文已自辩"三条都是同类命令、非短路反模式"，你需要判断这个自辩是否成立（反模式的本质风险是"后面的校验被跳过而未被发现"，三条 pytest 分片是否存在这个风险）。
7. **files_to_read 是否精简、无冗余**：12 个条目是否都确有必要（对照 dispatch-context 约束 8"不超过约 10 个文件"）。
8. **dispatch_plan 声明合法性**：`mode: serial` + 4 batches，核对是否符合 `agate/rules/dispatch.yaml`（或 review-mapping.md 附近）对 `dispatch_plan` 字段契约的要求（mode 枚举值、batches 结构）。

### 上游关联

- P1-requirements.md / P1-review.md（验收基线 + 已批准的红灯边界要求）
- P0-brief.md（范围基线）
- C8 映射已确认本任务只需你一人评审（backend + medium，非 full/high，无 NEED_CONFIRM）

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0035-gate-robustness/P2-design.md（评审对象）
- {AGATE_WORKSPACE}/tasks/TAG0035-gate-robustness/P1-requirements.md（验收基线核对）
- agate/scripts/check-gate.py（核实 gate_p4/gate_p8/handlers/回退检测的现状代码是否与设计描述一致）
- agate/scripts/check-state-transition.py（核实 phase_num/main() 消费点现状）
- agate/scripts/pre-commit-gate.py（核实 _P_OUTPUT_RE/_phase_num 现状，重点验证约束2）
- agate/scripts/check-judge-verdict.py（核实 _BLACKLIST_MD/_check_blacklist/_is_whitelisted 现状，重点验证约束4）
</dispatch_guide>
...
- 主 Agent 已核实 check-state-transition.py 的 `get_old_phase()`（L79起）文档自陈"git show 失败回退空"，即 `old_phase` 合法可以是空字符串（如任务首次阶段转移、无 HEAD 版本时）——这支撑了设计中"old_phase 为空字符串时 old_num=0 保留合法语义"的处理，评审时这一点可直接采信，不必重复核实
- 主 Agent 已核实 check-gate.py 当前代码：`handlers.get(phase) is None` 分支在 main() 尾部（约 L1486-1489），回退检测在其之前（约 L1464-1472），行号与设计描述基本一致（设计写的具体行号可能有 ±3 行浮动，属正常代码演进误差，不构成设计缺陷）
</objective_info>
