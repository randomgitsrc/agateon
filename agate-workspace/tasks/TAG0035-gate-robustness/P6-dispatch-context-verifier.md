...
> 本次是 P6 首次派发（非重试）。本任务无 UI（`domains: [backend]`，`ui_affected: false`），不需要 vision-analyst，不需要截图/vision 证据。`change_type` 未声明 refactor（功能修复任务），走标准 BDD 逐条验收口径，不走 refactor 回归口径。

### 目标

逐条重新执行 P1-requirements.md 的 14 条 BDD 对应的验证命令（每条 BDD 原文已给出"验证："这一行，且 P3-test-cases.md §2 已给出精确的测试函数名映射），产出 `P6-acceptance.md`（14 条 PASS/FAIL）+ `P6-evidence/`（每条 PASS 的证据文件）。

### 约束

1. **逐条独立验证，不是重跑一次全量测试就完事**：14 条 BDD 中，10 条对应 P3 新增的具体测试函数（`test_tag0035_bdd_N_*`），可用 `pytest -k tag0035_bdd_N -v` 精确单独跑并把输出保存为该 BDD 的证据；另外几条（如 BDD-1 的"未知阶段 exit=1"）P1 原文还给出了独立的手工 CLI 验证命令（如 `python3 agate/scripts/check-gate.py P99 ...; echo exit=$?`），**两种证据都可以用**，但每条 BDD 的证据必须能独立支撑该条判定，不能只用一句"全量测试通过"笼统带过全部 14 条。
2. **证据文件组织建议**：可以把 14 条 BDD 的 pytest 输出整理进少量几个证据文件（如按子批分 4 个文件：`P6-evidence/bdd-batch-a.log`/`bdd-batch-b.log`/`bdd-batch-c.log`/`bdd-batch-d.log`，每个文件跑 `pytest -k "tag0035_bdd_N or tag0035_bdd_M or ..." -v`），每条 PASS 行引用对应文件；也可以每条 BDD 单独一个文件，你自行判断，只要每个证据文件确有实质内容（不是 1 行文本充数）。
3. **PASS 行格式**：严格 `- PASS BDD-NN: {描述} ({证据路径})`，证据路径相对于 `P6-evidence/`（如 `(bdd-batch-a.log)`）。**不允许**多条 BDD 只共用一句总结当作证据——即使多条 BDD 引用同一个日志文件，也要求该文件里确实包含了对应到每条 BDD 的可辨识输出（如该测试函数名 + PASSED 字样）。
4. **post-test 环境残留检查**：本任务全部验证是本地文件系统 + 本地 git 命令，不创建外部资源（数据库/服务/网络连接），残留检查 = 确认没有遗留的临时 git 仓库/进程（pytest 用的临时 fixture 目录由 pytest 自身清理，无需手工检查），在 P6-acceptance.md 中写一句"本任务测试均为本地文件系统操作，无外部资源创建，无残留"即可满足该强制步骤，不需要额外脚本。
5. **frontmatter**：`pass: 14`（若全部通过）、`fail: 0`、`ui_affected: false`。
6. **不得改动任何源代码**：P6 是 self-authored gate，验收阶段暂存区只能有 `P6-evidence/` 下的证据文件 + `P6-acceptance.md` 本身，发现任何 BDD 实际 FAIL（不应该发生，P4/P5 均已确认全绿，但如果你验证时发现真的有 FAIL，如实写 FAIL，不要去改代码让它变绿——按角色文件流程报告主 Agent）。
7. **不需要引用 P5 证据复用机制**（BDD-12/13 那套 `p5_pass_commit` 审计7 机制仅适用于 `change_type: refactor` 任务的"全量回归全绿"证据豁免），本任务走标准逐条验收，正常独立产出证据。

### 上游关联

- P1-requirements.md（14 条 BDD 原文，含各自"验证："命令，权威验收标准）
- P3-test-cases.md §2（BDD→测试函数名映射表，直接可用）
- P5-test-results/unit.md（P5 已确认全量测试 0 failed，可作为背景参考，但本阶段仍需逐条独立产出 BDD 级证据，不能只引用 P5 的全量汇总）
- P4-implementation.md / P4-implementation-{B,C,D}.md（实现细节，供理解每条 BDD 对应的代码改动）

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0035-gate-robustness/P1-requirements.md（14 条 BDD 原文 + 验证命令）
- {AGATE_WORKSPACE}/tasks/TAG0035-gate-robustness/P3-test-cases.md（BDD→测试函数映射）
- {AGATE_WORKSPACE}/tasks/TAG0035-gate-robustness/P5-test-results/unit.md（背景参考）
</dispatch_guide>
...
- 当前 git HEAD：ffa0341（P5 commit），工作树干净
- 14 条 BDD 的测试函数名（来自 P3-test-cases.md §2，可直接用于 `pytest -k` 过滤）：
  BDD-1: test_tag0035_bdd_1_unknown_phase_fail_closed_exit_1
  BDD-2: test_tag0035_bdd_2_ci_backstop_exit_code_comparison_unaffected
  BDD-3: test_tag0035_bdd_3_known_phase_not_routed_to_unknown_path
  BDD-4: test_tag0035_bdd_4_retreat_detection_non_numeric_phase_fail_closed
  BDD-5: test_tag0035_bdd_5_phase_num_non_numeric_fail_closed
  BDD-6: test_tag0035_bdd_6_pre_commit_nonstandard_phase_output_warns
  BDD-7: test_tag0035_bdd_7_check_gate_numeric_retreat_detection_not_regressed /
         test_tag0035_bdd_7_state_transition_numeric_phase_not_regressed /
         test_tag0035_bdd_7_pre_commit_standard_phase_output_no_extra_warning
  BDD-8: test_tag0035_bdd_8_gate_p4_history_scan_prior_code_commit_allows_pure_md
  BDD-9: test_tag0035_bdd_9_gate_p4_retreat_fix_commit_with_retries_allows_pure_md
  BDD-10: test_tag0035_bdd_10_gate_p4_pure_doc_no_history_still_blocks
  BDD-11: test_tag0035_bdd_11_blacklist_exempts_phase_card_path_reference
  BDD-12: test_tag0035_bdd_12_whitelist_role_definition_file_path
  BDD-13: test_tag0035_bdd_13_p6_evidence_bare_filename_recognized
  BDD-14: test_tag0035_bdd_14_self_referential_p6_acceptance_still_blocked
