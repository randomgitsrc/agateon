...
把 `P0-brief.md` 已核实的 4 个子批（A/B/C/D）转化为正式 `P1-requirements.md` 需求基线：每个子批至少 1 条 BDD（Given/When/Then），frontmatter 声明齐全，含本阶段强制的「同类扫描」结论与「P0-brief 时效性质疑」记录。

### 约束

1. **子批 B 只写最小案**：P0-brief 已裁定"完整案"（`phases.yaml` 加 `order`/`predecessors`）与本批 out-of-scope「不做 phase 语义重构」冲突，**本批 BDD 只覆盖最小案**——非数字阶段名在三处（`check-gate.py` 回退检测 / `check-state-transition.py:phase_num` / `pre-commit-gate.py:_P_NUM_RE`）触发**显式报错**，不要写涉及 `phases.yaml` schema 改动的 BDD。完整案若你认为仍有必要，写成 `[SUGGEST: ...]` 留给 P2 定案，不要当作本批交付项。
2. **子批 D 范围已收窄**：只做 DEBT0038（`check-judge-verdict.py` 黑白名单 3 处假阳性）。DEBT0040/DEBT0041 **禁止**出现在本批 BDD 里（P0-brief 已注明移出理由：DEBT0040 涉及 CI workflow 改动需用户许可，DEBT0041 与 gate 判据健壮性仅名义相关且与 TAG0036 字段族有交集）。
3. **不做 `handlers` 字典声明式化**——P0-brief out-of-scope 已锁定，10 个手写 `gate_pN` 改 YAML 驱动是独立议题，不要在 BDD 里夹带。
4. **regression 测试必须逐子批声明为验收条件的一部分**（不是留白到 P4 才补）：至少要能在 BDD 里看出"改完后跑什么命令、断言什么退出码/输出"。
5. **同类扫描是本阶段强制节，不能只复制 P0-brief 已有结论**——P0 的三处数字序号假设（子批 B）本身就是一次同类扫描的产出，P1 要在此基础上**再扫一轮更大范围**：
   - 子批 A：`check-gate.py` 以外，其余 `check-*.py` 脚本是否也存在"未知枚举值 fail-open（返回等同通过的 exit code 或默认真值）"的同构模式？建议起点：`grep -rn "sys.exit(2)\|return 2\b" agate/scripts/check-*.py` 逐个排查是否在错误分支上。
   - 子批 D：`check-judge-verdict.py` 的黑白名单机制里，是否还有其他子串匹配（而非路径匹配）导致同类假阳性/假阴性风险的位置？
   - 结论写进 P1-requirements.md 正文（每条命中"本次处理/本次不处理+理由"），即使结论是"未发现更多同类实例"也要写出扫描命令与命中数。
6. **P0-brief 时效性质疑**：本 worktree 于 2026-09-16 完成 setup 并立即进入 P1，与 P0 立项（同日）间隔极短，主 Agent 已核对 P0-brief 三条严重漂移判据均不成立（技术路线未变/工具链未变/`known_risks` 前提未被其他任务解决——TAG0036 尚未启动，无范围重叠）。**你仍需在 P1-requirements.md 正文写一行"已核对 P0-brief 时效性，无漂移"**（不能省略这一步，即使结论显而易见）。
7. **frontmatter 建议值**（可据实际调整，但需给出理由）：`risk_level: medium`（参照 TAG0031/TAG0034 先例：改核心 gate 脚本、影响面覆盖 P1-P8 全阶段判定，不适用 low）；`phases: [P1,P2,P3,P4,P5,P6,P7,P8]`（不裁剪，理由同 risk_level）；`domains: [backend]`；`packages: [agate-scripts, agate-tests]`（若子批 D ②涉及改 `dispatch-protocol.md`/P6 卡，`packages` 需补 `agate-docs`）。`judge.enabled: true` 已在 `.state.yaml` 写好（本任务 created 2026-09-16 ≥ judge_required_since 2026-08-22，机制强制），不需要你改，但 P1-requirements.md 正文提一句确认即可。
8. **无 frontend 域**：本任务不涉及用户可见页面/UI，不需要 UX 类别 BDD / `ui_render_shape` 声明。
9. **BDD 编号全局唯一**，四个子批的 BDD 依次编号（BDD-1... 不要各子批各自从 1 编号）。

### 上游关联

- P0-brief.md 已完成两轮独立评审修正（子批 D 范围收窄、DEBT0041 排除），本次 P1 的 4 个子批范围以 P0-brief **当前版本**（含主 Agent 刚做的 known_risks 交叉引用修正）为准，不要再往回加 DEBT0040/0041。
- HANDOFF-TAG0035.md（worktree 根）：双工作区纪律、验证命令清单、阶段推进纪律（commit phase 语义、TDD 策略、SELF-GATE 触发面）——写 BDD 验收条件时可直接复用其中列出的验证命令作为判据。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0035-gate-robustness/P0-brief.md（权威范围来源，4 子批 + out-of-scope + known_risks 已写清）
- /home/kity/oclab/agateon/.worktrees/agate-TAG0035/HANDOFF-TAG0035.md（worktree 根，验证命令 + 阶段纪律）
- agate/scripts/check-gate.py（子批 A/B/C 涉及，行号见下方客观查证信息）
- agate/scripts/check-state-transition.py（子批 B）
- agate/scripts/pre-commit-gate.py（子批 B）
- agate/scripts/check-judge-verdict.py（子批 D）
- agate/rules/dispatch.yaml（judge_required_since 字段位置）
</dispatch_guide>
...
<objective_info>
主 Agent 本轮实测（均在 worktree 根 /home/kity/oclab/agateon/.worktrees/agate-TAG0035 执行，与 P0-brief 描述一致，行号可能因文件后续微调有 ±5 行浮动，以下为本次实测值）：

- 子批 A：`python3 agate/scripts/check-gate.py P99 agate-workspace/tasks/TAG0035-gate-robustness` → stderr `未知阶段: P99`，`exit=2`。`handlers` 字典定义在 check-gate.py:1474-1485，`func = handlers.get(phase)` 在 1486，未知分支 `sys.exit(2)` 在 1488 附近。
- 子批 B：
  - `check-gate.py:1465-1466`：`old_num = re.search(r"[0-9]+", old_phase)` / `new_num = re.search(r"[0-9]+", phase)`
  - `check-state-transition.py:212-215`：`def phase_num(text): ... m = re.search(r"[0-9]+", text); return int(m.group(0)) if m else 0`（docstring 自陈"无匹配回退 0"）
  - `pre-commit-gate.py:82`：`_P_NUM_RE = re.compile(r"P[0-8]")`（大小写敏感，且只认 P0-P8 单数字，与另两处 `[0-9]+`（可匹配多位数字/任意位置）**模式并不完全相同**——三处虽然都是"数字序号假设"同类问题，但正则本身不一致，写 BDD 时逐处验证，不要假设可以一次性统一改成同一个正则）
- 子批 C：`check-gate.py` 940-965 行区间：`_gate_p4` 里 `git diff --cached --name-only` 遍历，若无一行不匹配 `_STAGED_EXCLUDE_RE`（即暂存区全是 md/yaml），`return 1`；否则继续走后续维护性反模式判据。
- 子批 D：`check-judge-verdict.py` 第 63 行黑名单列表含字符串 `"p6-acceptance.md"`（子串匹配，非路径匹配），`_check_blacklist` 定义在 166 行，`_check_whitelist_outside` 定义在 192 行。
- `agate/rules/dispatch.yaml:17`：`judge_required_since: "2026-08-22"`。TAG0035 P0-brief created 2026-09-16 ≥ 该日期，`.state.yaml` 已写 `judge.enabled: true`（P0 立项时已做，本轮无需改动）。
- 环境：python3 3.12.3 / pyyaml 可用 / pytest 9.0.3；分支 `feat/TAG0035-gate-robustness`；工作区干净（仅本轮 P0-brief.md 一处未提交的编辑）。
</objective_info>
