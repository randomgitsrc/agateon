...
对 `P1-requirements.md`（TAG0032 版本管理生命周期可用性批，RM-AG0058 整合 epic）做独立需求基线评审，产出 `P1-review.md`。你是独立视角，负责发现 analyst 的作者盲区：遗漏的隐含需求、不可二值判定的 BDD、混入 P1 的解决方案设计、裁剪/风险声明与实际不匹配。

**只审不写**——不改 P1-requirements.md；评审意见由主 Agent 回派 analyst 修改。

### 约束

- 产出文件 Header `status:` 初始 `draft`；评审完成后**必须**改为 `approved` / `rejected` / `needs-revision`（gate 读 status 字段，非返回摘要）
- 结论必须引用具体锚点：approved 须逐条列 BDD 编号 + 覆盖维度清单（数据/前端/多端/边界/兼容逐项标注）+ 逐个裁剪阶段理由 + 审声明核对记录。裸 "approved" / "BLOCKER=0" 不接受
- 本任务 **非 frontend**（domains = [backend, cli]），跳过 UI/UX 机制评审要点与 vision 能力声明检查
- **审声明特别说明**：P1 阶段暂存区尚无非 md/yaml 改动（P1 产出只有需求文档 + .state.yaml），无 `git diff --cached` 代码证据可核。改为核对：analyst 声明的 `risk_level: high` / `ceremony: standard` / `phases: [P1..P8]`（不裁任何阶段）/ `implicit_coupling: true` 是否与 P0-brief `scope` + P1 §1/§4 描述的改动性质（改 3 个核心解析脚本 + 5 个文档面文件 + 新增测试；resolve 返回值语义变更牵动多消费方）匹配。声明与改动性质不符 → 结论 `needs-revision` 或 `rejected`，不得 approved
- 重点核查项（本任务高风险面）：
  1. **纯增量红线**：BDD-7（「根即协议」部署方解析语义不变）是否足以锁死 P0-brief「行为纯增量」约束；探测顺序（`vdir/scripts` 先、`vdir/agate/scripts` 后）是否在 BDD 中明确
  2. **BDD 二值判定**：BDD-2（三步迁移指引文案）、BDD-12（文档面无矛盾残留表述）这类"文案/一致性"判定是否可被明确 PASS/FAIL，判据是否够具体（BDD-2 已列"三者缺一即 FAIL"，BDD-12 已挂 `check-protocol-consistency --strict-errors-only` 0 ERROR——核对这类锚点是否真的可机械判定）
  3. **端到端可验证性**：BDD-13/14 依赖真实 GitHub 元仓库 clone，CI 无网降级为 `AGATE_REPO_URL` 注入本地 fixture repo + skip 标记 + P6-evidence 补证（§5 verification_env）——降级路径是否闭环、是否会让端到端形同虚设（TAG0008 教训：install 与 resolve/gate 从未串成一条链）
  4. **隐含需求覆盖**：§2 的 I-1（version 号不回归）、I-4（文档 doc/reality mismatch 须收敛不只新增）、I-11（install 拒绝不留半成品）是否完整；有无遗漏维度（数据/多端/边界/兼容——本任务无前端）
  5. **范围锁定**：P1 是否严守 P0-brief `out-of-scope`（pack-offline 适配 / Windows 复制模式专项 / .state.yaml schema 扩字段 / agateon 仓库形态重构）——§4 扫描 1 把 `install-offline.py:204` 判为"本次不处理转 roadmap"是否合理，还是其实应纳入本批
  6. **P1 纯净性**：§1 各断点的"修复行为"描述是否越界到方案设计（P1 应只定验收行为，把 resolve 探测 vs install 提升、副本 vs 软链明确留给 P2-design——核对是否确实留白）

### 上游关联

P0-brief 已锁定三段 scope + 4 条 known_risks + out-of-scope 四项。RM-AG0058（roadmap，scheduled）是立项依据。P1-requirements.md 由 analyst 产出：14 条 BDD（断点一 5 条 / 断点二 4 条 / 断点三 3 条 / 端到端 2 条），[NO_NEED_CONFIRM]，risk_level: high。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-requirements.md（评审对象，权威）
- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P0-brief.md（需求边界：scope / out-of-scope / known_risks）
- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-progress.md（analyst 分析留痕，辅助判断覆盖度）
- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/.state.yaml（phase=P1，judge.enabled=true）
- /home/kity/oclab/agateon/agate/assets/review-roles/requirements-review.md（你的角色定义，稳定版）
- /home/kity/oclab/agateon/.worktrees/agate-TAG0032/HANDOFF-TAG0032.md（交接单：三断点证据 + 核心约束 + 隔离 HOME 纪律）
- /home/kity/oclab/agateon/.worktrees/agate-TAG0032/agate/scripts/resolve-entry.py（核对 BDD-8 说的 `os.path.join(root, "scripts", gate_py)` 命中点是否属实，68 行）
- /home/kity/oclab/agateon/.worktrees/agate-TAG0032/agate/scripts/agate-install.py（核对 §4 扫描 1 说的 `:134` makedirs 穿透点，434 行）
- /home/kity/oclab/agateon/.worktrees/agate-TAG0032/agate/scripts/agate_common.py（核对 §4 扫描 2 说的 `_resolve_version_info` 归口 + 消费点，1050 行；按需 grep 片段，不必整读）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agateon/.worktrees/agate-TAG0032/agate-workspace`；改造对象仓库本体 = `/home/kity/oclab/agateon/.worktrees/agate-TAG0032`；读角色/卡片用稳定版 `/home/kity/oclab/agateon/agate`（= ~/.agate）。

### 客观查证信息（本机实测，硬约束）

- 解释器用 `/usr/bin/python3`；核对代码行号用 read/grep 工具，不改任何文件
- 若要跑 `check-protocol-consistency.py` 核对 BDD-12 可行性，用 **worktree 自己的**：`/usr/bin/python3 agate/scripts/check-protocol-consistency.py --strict-errors-only`
- bash 一律外层 `timeout`（30-90s）；单步串行不并行；状态标记 `[PROD_NOT_TOUCHED]`
- 产出路径硬约束：写 `{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-review.md`
- 分阶段落盘：发现逐条追加 `{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-review-progress.md`
- 产出 frontmatter 用 `/usr/bin/python3 agate/scripts/agate-md-field-set.py`（先 `--list`），不手写。Header 成品值：`phase: P1` / `task_id: TAG0032` / `parent: P1-requirements.md` / `trace_id: TAG0032-P1review-20260907` / `agent: requirements-review` / `type: review` / `status: draft`（评审后改终值）

### 返回

`File: <P1-review.md 路径>` + `Status: <approved|rejected|needs-revision>` + 一句话摘要（≤30 字）。不返回文件全文。
</dispatch_guide>
