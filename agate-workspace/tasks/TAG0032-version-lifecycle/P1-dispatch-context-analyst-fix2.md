...
<dispatch_guide>
> ⚠️ 派发指引是强制指令。本轮是 **P1 gate 格式微修**（不改任何实质内容）。

### 背景

P1-review 已 approved，但预跑 `check-gate.py P1` exit 1：
`GATE P1: 不合规的 NEED_CONFIRM 标记格式`。
根因：`P1-requirements.md` §8「待确认清单」段末出现字面 token `` `[NEED_CONFIRM]` ``（反引号包裹，行中）——
check-gate.py:692 检测到文中含 `[NEED_CONFIRM]` 字串但无行首阻塞标记 → 判为不合规格式。
analyst 角色文件亦明确「不要写『无 [NEED_CONFIRM]』」。

### 唯一修复动作

编辑 `{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-requirements.md` §8 最后一句：
把 `……未触碰 `out-of-scope`（……agateon 仓库形态重构），无 `[NEED_CONFIRM]`。`
改为不含字面 token `[NEED_CONFIRM]` 的等义表述，例如：
`……未触碰 `out-of-scope`（……agateon 仓库形态重构），无阻塞性待确认项。`

- 只改这一处措辞；**不动** §8 开头的行首 `[NO_NEED_CONFIRM]` 声明（那是合规的负向声明，必须保留）
- 全文其它任何地方不得改动；frontmatter 不动
- 改完自查：`grep -nE 'NEED_CONFIRM|SUGGEST' P1-requirements.md` 应只剩 L21 `[NO_NEED_CONFIRM]` + §8 行首 `[NO_NEED_CONFIRM]` 两处，无裸 `[NEED_CONFIRM]`
- 改完跑 `/usr/bin/python3 agate/scripts/check-frontmatter.py <路径>`，非 0 先修
- 状态标记 `[PROD_NOT_TOUCHED]`

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-requirements.md（修改对象，§8 在文件末尾）

路径：{AGATE_WORKSPACE} = `/home/kity/oclab/agateon/.worktrees/agate-TAG0032/agate-workspace`。

### 返回

只返回两行：① P1-requirements.md 路径；② 一句话摘要（≤20 字）。
</dispatch_guide>
