...
> 本轮是 **P1 修复轮（retry #1，needs-revision）**——增量模式，只改 P1-review.md 点名的 3 项必修 + 择优采纳建议项，不重写全文。

### 上轮产出与评审

- 上轮产出：`{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-requirements.md`（14 条 BDD，已 check-frontmatter 通过）
- 上轮 dispatch-context（复用其全部约束）：`{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-dispatch-context-analyst.md`
- 评审意见：`{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-review.md`（status: needs-revision）——**先完整读它的「结论」节**

### 修复目标（3 项必修，逐条落地到 P1-requirements.md）

1. **BDD-12 机械判据在基线恒真**：`check-protocol-consistency.py --strict-errors-only` 在修复前基线即 EXIT=0 / 0 ERROR（评审实测），"0 ERROR" 子判据不验证 doc/reality 矛盾是否收敛。
   → 重写 BDD-12：把 §4 扫描 4 已列的具体矛盾表述逐条列成**可枚举 checklist**，每条给二值断言「已收敛到新口径 / 已标注为版本历史叙事保留」。至少覆盖：UPGRADING L546 表格「根含 `scripts/`」承诺（doc/reality mismatch）、L552 升级入口表述、v0.60/0.61/0.62 节 vs v0.66/0.67/0.68 节的 hook 重装口径分歧。`check-protocol-consistency --strict-errors-only 0 ERROR` 降为**附加回归项**（防新增 ERROR），不作主判据。
2. **I-3 无 BDD 绑定**：I-3 自述「所选（根 `scripts/` 副本 vs 软链）语义须有单测锁定 + 写入 UPGRADING」，但 BDD-4 / BDD-11 均未承接。
   → 在 BDD-4 或 BDD-11 的 Then 增一条二值可判断言：所选根 `scripts/` 建立方式的维护语义（升级期是否需重跑 install）**有单测锁定** 且 **写入 UPGRADING「版本管理生命周期」节**。（P1 仍不决策副本/软链——那是 P2-design；P1 只锁"所选语义须有单测 + 落文档"这个验收行为。）
3. **BDD-2 判据宽严不一**：「备份软链」判据具体到命令片段（`mv ~/.agate ~/.agate.bak`），「建目录根」「装版本」仅「可定位关键点」。
   → 把后两项也收敛到命令片段级 **或** 明确的必现关键词清单，使「三者缺一即 FAIL」真正可机械判定。

### 建议项（非阻塞，择优采纳）

- BDD-7 Then 对称补「`AGATE_VERSION` 仍为 `v<X.Y.Z>`」（与 BDD-6 对齐）
- BDD-13 Then「每一步 exit 符合预期」逐步枚举各步期望 exit code（含幂等复跑期望 exit 0 且不重复建目录）
- §4 扫描 4 行号漂移校正：文中 `UPGRADING.md:551/552` → 实际表格约 L546-L552

### 约束（不变，重申关键项）

- **不扩范围**：3 项必修都在既有 BDD / 隐含需求范围内微调，不新增断点、不触碰 P0-brief `out-of-scope`
- **不改已确认合规项**：BDD 编号连续性、同类扫描四类、审声明 7 项、纯增量红线 BDD-7 探测顺序、时效性质疑——评审已确认，不要动
- 改动集中在 P1-requirements.md 的 §3（BDD-2/4 或 11/7/12/13）+ §2 表格 I-3 行「对应 BDD」列（如有）+ §4 扫描 4 行号；frontmatter 不动
- 改完返回前跑 `/usr/bin/python3 agate/scripts/check-frontmatter.py {P1-requirements.md 路径}`，非 0 先修
- BDD 编号保持连续、`#### BDD-NN:` 格式；若重写 BDD-12 为 checklist 形式，仍保留 `#### BDD-12:` 标题 + 单一 Given/When/Then 外壳（checklist 作为 Then 的可枚举判据项）
- 状态标记 `[PROD_NOT_TOUCHED]`

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-review.md（评审意见，权威——必修 3 项的判据在「结论 > 必修项」节）
- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-requirements.md（修改对象）
- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-dispatch-context-analyst.md（上轮完整约束，复用）
- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P0-brief.md（范围边界）
- /home/kity/oclab/agateon/.worktrees/agate-TAG0032/agate/UPGRADING.md（核对 L546/L552 表格 + v0.60-0.62 / v0.66-0.68 节的 hook 重装口径原文，供 BDD-12 checklist 逐条列准）
- /home/kity/oclab/agateon/agate/assets/execution-roles/analyst.md（角色定义，稳定版）

路径：{AGATE_WORKSPACE} = `/home/kity/oclab/agateon/.worktrees/agate-TAG0032/agate-workspace`；改造对象仓库 = `/home/kity/oclab/agateon/.worktrees/agate-TAG0032`。

### 客观查证信息（不变）

- 解释器 `/usr/bin/python3`；bash 外层 `timeout` 30-90s；单步串行不并行
- 产出路径硬约束：改写 `{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-requirements.md`（原地更新，不新建）
- frontmatter 用 `agate-md-field-set.py`，不手写；本轮 frontmatter 无需改动
- 分阶段落盘：进展追加 `{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-progress.md`

### 返回

只返回两行：① P1-requirements.md 路径；② 一句话摘要（≤30 字，说明 3 项必修各如何落地）。
</dispatch_guide>
