...
> 本轮是 **P1 复评轮**（首轮 needs-revision，analyst 已回改）——聚焦核查 3 项必修是否真正落地 + 回归确认未引入新问题，不重走全量评审。

### 上轮评审与本轮回改

- 你的首轮评审：`{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-review.md`（status: needs-revision，3 项必修 + 3 项建议）——**先读它的「结论」节**
- analyst 回改后的产出：`{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-requirements.md`（已 check-frontmatter 通过，BDD-1..14 仍连续）
- analyst 自述落地方式：BDD-12 重写为 4 条二值 checklist（0 ERROR 降附加回归项）；BDD-4 + BDD-11 承接 I-3（单测锁定 + 写入 UPGRADING）；BDD-2 三项判据统一到命令片段级；建议项（BDD-7 对称补 AGATE_VERSION、BDD-13 逐步枚举 exit、§4 扫描 4 行号校正）全采纳

### 本轮核查项（逐条给 PASS/FAIL + 锚点）

1. **BDD-12**（§3.3，L157 起）：是否已从「基线恒真的 0 ERROR」改为可枚举 checklist？逐条核对——(a) checklist ≥3 条具体矛盾表述，每条二值断言「已收敛到新口径 / 已标注为版本历史叙事保留」二选一无第三态；(b) `check-protocol-consistency 0 ERROR` 是否明确降为附加回归项（非主判据）；(c) 每条断言是否真的可机械判定（如「install 后 `~/.agate/scripts/` 存在」= 可判；「口径一致」若无具体锚点仍是主观 → 打回）
2. **I-3 承接**（BDD-4 L107 起 + BDD-11 L149 起）：是否有二值可判断言锁定「所选根 scripts/ 建立方式的维护语义有单测锁定 + 写入 UPGRADING 生命周期节」？BDD-4 判据 2/3 与 BDD-11 判据 3 是否交叉一致、不自相矛盾？P1 是否仍未越界决策副本/软链（应只锁「所选语义须有单测 + 落文档」的验收行为）？
3. **BDD-2**（L94 起）：三项判据（备份软链 / 建目录根 / 装版本）是否已统一到命令片段级或明确必现关键词清单？「三者缺一即 FAIL」是否现在真正可机械 grep 判定？
4. **回归确认**：BDD-1..14 编号仍连续、`#### BDD-NN:` 格式；改动是否只落在 §3 相关 BDD + §4 行号，未误伤首轮已确认合规项（同类扫描四类、审声明 7 项、纯增量红线 BDD-7 探测顺序、时效性质疑、范围锁定 out-of-scope）；BDD-12 checklist 重写后是否仍保留单一 Given/When/Then 外壳（checklist 作为 Then 的判据项）；有无新增断点或悄悄扩范围

### 结论口径

- 3 项必修全部落地且可机械判定 + 无回归 → `status: approved`（正文逐条列 BDD 编号 + 覆盖维度 + 3 项必修的落地核对记录 + 审声明复核一句话）
- 任一必修未真正落地 / 引入新问题 → `status: needs-revision`（明确指出哪条、差在哪、怎么改）
- 首轮已确认合规的项不必重复长篇复核，一句话带过即可

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-review.md（你的首轮评审，权威基准）
- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-requirements.md（回改后的评审对象）
- {AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-dispatch-context-analyst-fix1.md（回改派发指引，了解要求了什么）
...
- /home/kity/oclab/agateon/.worktrees/agate-TAG0032/agate/UPGRADING.md（核对 BDD-12 checklist 逐条引用的 v0.50.0 / v0.60-0.62 / v0.66-0.68 节表述是否属实、行号是否对得上）

路径：{AGATE_WORKSPACE} = `/home/kity/oclab/agateon/.worktrees/agate-TAG0032/agate-workspace`；改造对象仓库 = `/home/kity/oclab/agateon/.worktrees/agate-TAG0032`；读角色/卡片用稳定版 `/home/kity/oclab/agateon/agate`。

### 客观查证信息（不变）

- 解释器 `/usr/bin/python3`；核对用 read/grep，不改任何文件
- 跑 `check-protocol-consistency.py` 用 worktree 自己的：`/usr/bin/python3 agate/scripts/check-protocol-consistency.py --strict-errors-only`
- bash 外层 `timeout` 30-90s；单步串行；状态标记 `[PROD_NOT_TOUCHED]`
- 产出路径硬约束：写 `{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-review.md`（**原地覆盖**首轮评审——同一文件，更新 status + 正文为复评结论）
- Header 用 `agate-md-field-set.py` 更新：`trace_id: TAG0032-P1review-re1-20260907`，`status` 改终值（approved / needs-revision）
- 分阶段落盘：追加 `{AGATE_WORKSPACE}/tasks/TAG0032-version-lifecycle/P1-review-progress.md`

### 返回

`File: <P1-review.md 路径>` + `Status: <approved|needs-revision>` + 一句话摘要（≤30 字）。不返回文件全文。
</dispatch_guide>
