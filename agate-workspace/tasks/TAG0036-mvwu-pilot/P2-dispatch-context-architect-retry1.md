...
> 本次是 P2 **重试 #1**（plan-eng-review 结论 rejected：2 个阻塞问题，共享 P2 retry 预算 MAX=3）。在原 `P2-design.md` 上**就地修订**，不重写；首次派发的约束（`P2-dispatch-context-architect.md`）继续有效。

### 目标

按 `P2-review.md` 修订 `P2-design.md`：**B1、B2 必改**，**N1-N4 一并处理**（N2 可改批 id 或在 §6 说明理由；N5/N6 无需改）。

### 约束

1. **先读 `P2-review.md` 全文与现有 `P2-design.md`**，逐条对照修订；评审判"通过"的部分不动。
2. **B1（主 Agent 已裁决采纳评审建议）**：新增 **M18** = `agate/scripts/check-protocol-consistency.py::GATE_SCRIPT_EXEMPT` 加入 `"agate/scripts/check-mvwu.py"` 一行（注释「观测脚本，不挂 gate」）。**我已批准并在 P1 落 `[BASELINE_CHANGE]`**（见 `P1-requirements.md` BDD-70 下方 + §4.3 表该行）——该文件**不在**零内核清单，此改动合法；但**不要**改 `SCRIPT_ALIGNMENT_ANCHORS` 锚点表，也不要改该文件其他任何内容。同步设计：
   - M18 并入 B1 批 `mvwu-verdict-observer`（`check-mvwu.py` + `check-protocol-consistency.py` = 2 文件，仍 ≤3）；批的 complexity 是否仍 medium 由你判断；
   - §0.1 增 M18 行；§0.2 把 `check-protocol-consistency.py` 一行从 Not Modify 移出并改述；R2 改为「已缓解」（P5 `CHECK9-coverage` 预期 **0 新增**、`test_sg_6` 为绿）；§4 BDD-70 行加落点 M18；§5、§7 P5 验收口径、§8、§12 第 3 条、CHANGELOG 措辞去掉"预期 +1 WARNING"；**§8 原 `[SUGGEST]`（接受 +1 WARNING）作废**，改写为说明"已由主 Agent 裁决为加 GATE_SCRIPT_EXEMPT"（**不要**再写 `[SUGGEST` 字样，避免 gate 格式检查误伤）；确认 `P5_kernel_diff(_wt)` 清单**不含** `check-protocol-consistency.py`；
   - 新增风险条目：「既有测试对新增脚本的反应」（`test_sg_6` 一类"glob 全覆盖"断言），配缓解（P3 把 `test_sg_6` 列为"P4 前应已绿的守护用例"，非新增测试）；再全仓 grep 一次是否还有其他"对 `agate/scripts/check-*.py` 全量 glob 的既有测试/检查"会被新脚本触发（评审只实测了 test_sg_6 与 CHECK9 两处）——**须实测抽样**：在 `/tmp`/scratchpad 副本放空 `check-mvwu.py` 后跑 `agate/tests/` 里命中 glob 的相关测试（`grep -rn "check-\*.py\|glob(.check-" agate/tests`），把结论写进 `minimal_validation`（副本用后删除，不入库）。
3. **B2**：`P5_roles_diff` 与 `P5_history_untouched` 的值末尾 token 不得带引号（`agate-read-p5-commands.py:33` 会 strip 首尾引号）。按评审建议改写，改后**必须**用 `python3 ~/.agate/scripts/agate-read-p5-commands.py <P2-design.md>`（先看其 `--help`/用法）读回全部 key，并对每个值 `shlex.split` 自证无 `ValueError`、语义保持（pathspec 仍生效）；结果写入 `minimal_validation`。
4. **N1**：在 §6 波次说明追加「P4 各批 dispatch-context 强制写明波次并发上限 ≤3」（不改 `parallel_limit: 6`）。**N3**：修正 `files_to_read` 中 `P2-design.md` 卡片的行号引用（以标题定位为准，给标题名 + 实测行号）。**N4**：更正 §7 关于 `~` 令 gate 出 WARNING 的不准确措辞。**N2**：批 id `tests-filter-authoring-guidance` 与交付内容（含判据一/二/三 + 决策读取）名实不符——改为更贴切的能力名（如 `architect-batch-guidance`）并**同步 frontmatter `dispatch_plan` 与正文所有引用**，或在 §6 写明保留理由；若改名须用 `agate-md-field-get.py dispatch_plan` 读回核对。
5. **正文末尾追加「修订记录（retry #1）」**：B1/B2/N1-N4 逐条：处置（已改/未改+理由）+ 涉及章节。
6. **范围纪律不变**：仍不得改零内核清单；P2 阶段不改 `agate/` 任何文件（M18 是 P4 的设计落点，P2 只写设计）。`[NEED_CONFIRM]` 不允许留有未决项；无新 `[SUGGEST` 需要时用严格格式 `[SUGGEST: 推荐 X，理由 Y]`。
7. frontmatter 如需改只用 `agate-md-field-set.py`（不接受的键，如 `agent`/`dispatch_plan`，Edit 单行）；改完跑 `python3 agate/scripts/check-frontmatter.py <P2-design.md>`；`status` 保持 draft。

### 上游关联

- `P2-review.md`（rejected，B1/B2 含实测证据）；`P1-requirements.md`（含新增 `[BASELINE_CHANGE]`）；`P0-brief.md`

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P2-review.md
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P2-design.md（修订对象）
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P1-requirements.md（BDD-70 及其 BASELINE_CHANGE、§4.3）
- agate/scripts/check-protocol-consistency.py（`GATE_SCRIPT_EXEMPT` 一带，仅读）
- agate/scripts/agate-read-p5-commands.py（B2 复现，仅读）
- agate/tests/integration/test_protocol_alignment_review.py（`test_sg_6` 一带，仅读）
</dispatch_guide>
...
<objective_info>
- `P2-review.md`：status=rejected，agent=plan-eng-review；B1（test_sg_6 由绿转红，评审在 scratchpad 副本复现）、B2（两个 P5 key 末尾引号被 `agate-read-p5-commands.py` 吃掉，评审实跑复现）。
- 现 `GATE_SCRIPT_EXEMPT`（`check-protocol-consistency.py:805`）含 2 条：自身与 `pre-commit-gate.py`。
- 本地分支 `main` = `efb113b`。P2 retry 计数：第 1 次（MAX=3）。
</objective_info>
