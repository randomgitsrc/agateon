...
> 本次是 P5 首次派发（非重试），**P5 技术验证模式**。

### 目标

从 `P2-design.md` 的 `gate_commands` 读取并**逐条独立执行**全部 `P5*` 命令（不用 `&&` 串联；每条独立记录 exit code / 关键输出 / 耗时），产出 `P5-test-results/unit.md` 与 `P5-test-results/fail-list.txt`。

### 约束

1. **命令来源**：`P2_DESIGN={AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P2-design.md python3 ~/.agate/scripts/agate-read-p5-commands.py` 读回全部 key（P5 / _consistency / _ruff / _count / _windows_smoke / _debt / _kernel_diff / _kernel_diff_wt / _roles_diff / _history_untouched，共 10 条），**逐字**按读回值执行（`shlex.split` 后用 `subprocess`/等价方式，勿手改命令；`~` 需展开，`_ruff` 的 `~/.venvs/agate-dev/bin/ruff` 直接用绝对路径展开）。每条命令前设 shell 层 `timeout`（取 P2 声明的 `{key}_timeout_seconds`×1.5；`P5` 全量为 `P5_timeout_seconds: 600` 的×1.5）。**工作目录 = worktree 根**；`_consistency` 用 **worktree 自己的** `agate/scripts/check-protocol-consistency.py`（命令已是相对路径，勿改成 `~/.agate` 版）。
2. **判定口径（P2 §7 说明 / §12）**：`P5` 全量 pytest `--reruns 1 -n auto`：failed=0、skipped 仍为 2、passed ≥ 1666 + 新增用例（预期 1842）；`P5_consistency` exit 0（`--strict-errors-only`，**0 ERROR**，`CHECK9-coverage` 无新增 WARNING）；`P5_ruff` exit 0；`P5_count` 输出总数 = 1668 + 111（test_check_mvwu）+ 65（test_mvwu_protocol_docs）= 1844；`P5_windows_smoke` exit 0（Linux 上 `-m windows_smoke` 冒烟）；`P5_debt` exit 0；`P5_kernel_diff` / `P5_kernel_diff_wt` / `P5_roles_diff` / `P5_history_untouched` **均须 exit 0（空 diff）**。任一非 0 → 如实记录为失败（不要修代码、不要重跑到通过为止；flaky 重跑仅限 `--reruns` 内建）。
3. **只验证不修改**：不修改任何代码/文档/测试；临时文件放 tmp 后删除；不 git add/commit（主 Agent 处理）；`git status` 除 `P5-test-results/` 及进度文件外不得新增改动（注意：pytest 会写 `__pycache__`，已被忽略）。
4. **产出格式**：`P5-test-results/unit.md` 头部写 `phase: P5` / `agent: verifier` / `task_id: TAG0036` / `HEAD (父提交): <git rev-parse HEAD>` / `[PROD_NOT_TOUCHED]`；随后逐条命令一节：**原命令原文、exit code、关键输出摘录（pytest 汇总行须含 `passed`/`failed` 计数字样以便主 Agent 校验 runner 签名）、判定**；末尾「汇总表」+ 明确一行 `failed 总数: N`；含"全量测试已运行"声明（P5 是全量）。`P5-test-results/fail-list.txt`：failed 测试 id 逐行（`FAILED ` 前缀），无失败则为**空文件**。**无 UI**（`ui_affected: false`），不产出 e2e.md。
5. 预存失败：如发现与本任务无关的预存失败，在 unit.md 标注"预存失败：X（与本次改动无关）"并如实列出；本机基线 1666 passed / 2 skipped 无已知预存失败。
6. 命令超时/异常按 dispatch-prompt「命令超时兜底」处理：停止、写 progress、返回主 Agent，不自行换命令。
7. 分阶段落盘：每条命令执行**前后**往 `P5-progress.md` 用 `>>` 追加一行。

### 上游关联

- `P2-design.md` §7（gate_commands 与说明）、§12（完成标志）；`P4-implementation.md`；`P4-review.md` / `P4-protocol-alignment-review.md`（均 approved）

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P2-design.md（§7）
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P0-brief.md、P1-requirements.md（BDD 与口径，背景）
- ~/.agate/scripts/agate-read-p5-commands.py（读回 gate_commands）
</dispatch_guide>
...
<objective_info>
主 Agent 提交 P4（`4ee499f`）前本人实测：全量 pytest 1842 passed / 2 skipped；ruff All checks passed；consistency 0 ERROR（367 WARNING，无 CHECK9-coverage）；`check-platform-assumptions.py` 0 命中；`count-tests.sh` 总计 1844；四条 diff key 读回执行均 rc=0。以上是主 Agent 的预测试结果，**不能替代你的独立执行与记录**；如你的结果与之不同，以你的实测为准并如实记录差异。
</objective_info>
