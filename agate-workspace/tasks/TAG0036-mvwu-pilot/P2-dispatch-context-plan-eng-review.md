...
> 本次是 P2-design.md 首次评审（非重试）。你与 architect 是不同 agent（信息隔离），请独立判断，不要预设产出一定合格。

### 目标

独立评审 `P2-design.md`（TAG0036 MVWU 阶段 1 试点：3 个候选、选定单文件方案 A、6 批 static-batch、gate_commands 含 P3/P5/consistency/ruff/count/windows_smoke/debt/kernel_diff 等），判定 approved / rejected，写入 `P2-review.md`（域=backend，单评审角色，无需组长汇总）。

### 约束

评审重点（按本任务特点定制，须**实测抽样**，不能只读文档）：

1. **BDD 全覆盖与落点真实性**：P2 §4 声称 71 条 BDD 全部有落点。逐条抽查至少 10 条（含 BDD-22/30/34/47/50/57/62/66/70）：落点文件/小节是否真实存在且合理；有无 BDD 无落点或落点落在"零内核改动清单"文件上。
2. **零内核改动**：P2 是否设计了对 `check-gate.py` / `phases.yaml` / `rules/schema/` / hook 三件套 / `agate_common.py` / `check-state-*` / `check-events.py` / provenance / judge 校验等的任何改动；`check-mvwu.py` 对 `agate_common` 是否仅 import 使用。
3. **候选方案真实性**：A/B/C 是否是真实可行且权衡自洽的候选（不是陪衬）；选 A 的理由是否成立；有无遗漏的更简方案。
4. **口径未被悄悄改**：P1 口径 A-H（reason 词表 / 判定顺序 / 列表编码 / `|` 转义 / git 区间 / 首词语义 / `git_head` 格式）与 P2 §2 数据流是否一致；`check-mvwu.py` 是否被设计成会**执行** `tests_filter`/`command`（P1 BDD-19 禁止执行）。
5. **P2 §8 的 `[SUGGEST]`（CHECK9 多 1 条 WARNING）**：请**自己实测**——在 worktree 临时放一个空的 `agate/scripts/check-mvwu.py`（评审后**务必删除**，不得 git add）或在 `/tmp` 副本复现，确认 `check-protocol-consistency.py` 是否确实新增 1 条 `CHECK9-coverage` WARNING、`--strict-errors-only` 是否仍 exit 0；并评估"接受 vs 加 `GATE_SCRIPT_EXEMPT` 一行"哪个更合理、是否会破坏 P1 边界（P0 out-of-scope 未禁止改该文件，但 P1 §4.3 判不处理）。给出你的建议，主 Agent 做最终处置。
6. **批次设计**：6 批的 `dispatch_plan` 是否合法（`agate-md-field-get.py dispatch_plan` 读回）；批切分是否满足 P1 判据二（Vertical Slice：`id` 回答"交付了什么能力"）——两个登记类批（`mvwu-glossary-and-debt-log` / `mvwu-script-registry`）按技术面切的理由是否成立；波次依赖（CHECK 10：`check-mvwu.py` 先于引用它的文档）是否自洽；同一文件是否跨批两轮改动（逐批核 M1-M15 文件归属）。
7. **gate_commands 可执行性**（实测抽样）：`P5_kernel_diff` 的三点 diff `main...HEAD` 在本 worktree 能否解析（本地 `main` = efb113b 已核实存在）；路径清单里的每个文件是否真实存在（不存在的路径会让 diff 空转通过）；`P5_history_untouched` 的 pathspec 通配是否被 git 正确解释；`P5_ruff` 用 `~` 的 WARNING 可否接受；每个 key 是否独立、无 `&&` 串、无 `P3_xxx` 键；`P3` 的测试文件在 P2 时尚不存在——P3 的写法约定是否合理；timeout 声明是否合理。另外用 `python3 ~/.agate/scripts/check-gate.py P2 <task_dir>` 之外**不要**改任何文件（gate 此时会因缺 P2-review.md 返回 1，属预期；只看其他条目的输出）。
8. **最小验证**：`minimal_validation` 的验证（git 区间 / 参数化 id 解析 / `agate-md-field-get.py` 透传）是否真的做了且结论支撑设计；抽 1-2 条复现。
9. **dogfooding 的 `[SUGGEST]`（本任务自身批不写 `tests_filter`/`output`）**：理由是否成立；是否与 P0/P1"不主动造样本、n=1 偏差"一致。
10. **实现就绪度**：files_to_read 是否足以让 implementer 无需步骤脚本即可实现；有无把"执行顺序脚本"写进设计而浅化接口（Deep Modules 视角）；风险表 R1..Rn 是否各有缓解。

评审输出按角色文件的「输出结构」（架构问题-阻塞/非阻塞、测试缺口、锁定决策），并对 §8 与 §6 的两条 `[SUGGEST]` 各给"同意/不同意 + 理由"。**发现任何阻塞项 → status: rejected**。`agent: plan-eng-review`（≠ main）。只审不写——不改 P2-design.md、不 git add/commit；临时文件用后删除，确保 `git status` 除 `P2-review.md`/`P2-progress.md` 外无新增。

### 上游关联

- `P1-requirements.md` / `P1-review.md`（approved）；`P0-brief.md`；`P2-design.md`（评审对象）
- 设计文档 `docs/design-notes/design-mvwu-protocol.md`

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P2-design.md（评审对象）
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P1-requirements.md（BDD 基线）
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P0-brief.md
- agate/scripts/check-protocol-consistency.py（CHECK 9 / CHECK 10 一带）
- agate/scripts/agate-md-field-get.py、agate/scripts/check-gate.py（`_gate_p2_dispatch_plan` / `_gate_missing_cmds`）
...
<objective_info>
主 Agent 实测（worktree 根）：
- `P2-design.md` 413 行；`check-frontmatter.py` exit 0（architect 自检已过）；`dispatch_plan` 6 批 static-batch，`parallel_limit: 6`；`agate/` 下无 TAG0036 改动；本地分支 `main` = `efb113b`（存在）。
- `.state.yaml` 仍 phase=P1（P2 phase 推进随 P2 产出 commit）。
- architect 偏离派发指引 1 处：`gate_commands` 用 `python3 -m pytest`（本机无 `python` 命令）；理由有证据。请核对该偏离是否合理，及是否与 P1 BDD-57 "不裸 python3" 的适用范围（测试代码，不含 gate_commands）自洽。
</objective_info>
