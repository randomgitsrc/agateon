...
> 本次是 P1-requirements.md 首次评审（非重试）。你与 analyst 是不同 agent（信息隔离），请独立判断，不要预设产出一定合格。

### 目标

独立评审 `P1-requirements.md`（TAG0036 MVWU 阶段 1 试点，含 ①②③④ 四项 MVWU 交付物 + ⑤-a…⑤-e 方法学概念落地，共 58 条 BDD），判定 `status: approved / rejected / needs-revision`，写入 `P1-review.md`。

### 约束

评审重点（按本任务特点定制）：

1. **BDD 可二值判定 + 每条仅一组 Given-When-Then**：58 条是否都给出可执行验证命令/断言与明确的期望值；有无"多场景塞进一条"（应拆分）或"两条 BDD 实为同一断言"（应合并）的冗余；编号 `#### BDD-NN:` 是否连续不跳号。
2. **范围纪律（对照 P0-brief 的 out-of-scope，逐条核）**：是否夹带阶段 2（批级 gate）、改 `check-gate.py`/`.state.yaml` schema/`phases.yaml`/hook/审计链/P6.5/provenance/目录登记；是否把「Q1/Q3 已答」写成完成条件；是否替用户裁决「提交粒度」；⑤ 组是否越界（重构术语表、机械校验术语、规定架构适应度工具、强制垂直切分、决策自动过期 gate、引入部署/CI）。
3. **P1 纯净性**：BDD 是否规定了实现方式（函数名/内部结构/算法）而非行为；P1 只定义"做什么"。特别检查 `check-mvwu.py` 的 exit code 约定与 `--observe` 输出列结构——这些是**外部可观测契约**（允许出现在 BDD），但内部实现不得出现。
4. **analyst 三条 [SUGGEST] 的独立判断**（主 Agent 倾向采纳，但你须独立评估并在 review 中给出是否同意）：SUGGEST-1（`P4-evidence` 日志加可选键 `failed_tests`，供 EXPECTED_RED 判定）、SUGGEST-2（verdict 与 boundary 解耦——verdict 仅由六项检查决定，boundary 仅是 `--observe` 的观测列；与设计文档 §7.3 示例 C 行读法不同，须核对是否与 `P0-brief` 「四态 verdict 语义 / 合并提交下 boundary 判 UNKNOWN」自洽且不违背设计文档 §5.1.1）、SUGGEST-3（`output` 可选键仅供 `--observe` 读取）。若认为其中任一条属范围扩张或与设计文档冲突 → 在 review 中标 needs-revision 并指明改法。
5. **四态 verdict 覆盖与自洽**：`PASS`/`FAIL`/`EXPECTED_RED`/`UNKNOWN` 各态判定条件是否互斥且穷尽（有无某种输入落入两态或无态）；`UNKNOWN ≠ PASS` 是否有独立可判定 BDD；六项检查（且仅此六项）与 verdict 的映射是否清晰。
6. **同类扫描完整性（强制节）**：4.1~4.5 是否含扫描命令 + 命中数 + 逐条"处理/不处理+理由"；`dispatch_plan` 13 个消费方逐个判定是否合理；新脚本登记面清单（README 脚本表 / tests README 计数 / CHANGELOG / CHECK 10 等）是否完整——**请抽样实测至少 3 条命中数**（如 `grep -rIl tests_filter`、`grep -rIl dispatch_plan agate`、`bash agate/tests/scripts/count-tests.sh`），与 analyst 所称是否一致。
7. **引用事实核实**：BDD-54 列出的 16 个内核文件路径是否都真实存在（`ls` 核对，不存在的路径会让"diff 为空"空转通过）；BDD-56/57 引用的 `CHANGELOG.md [Unreleased]`、`count-tests.sh`、CHECK 10、`~/.venvs/agate-dev/bin/ruff` 是否存在；BDD-58 的 `P4-protocol-alignment-review.md` 命名是否与既有任务惯例一致（`ls agate-workspace/tasks/TAG0035-gate-robustness/`）。
8. **P0_STALE 4 条**：格式 `[P0_STALE: 具体漂移点]` 是否合规、严重/轻微判定是否合理（无严重漂移的结论是否成立）；主 Agent 已把 analyst 的第 3/4 条同步补记到 P0-brief「P0 自检记录」。
9. **frontmatter 声明自洽**：`risk_level: medium`、全阶段不裁剪、`packages`/`domains` 与实际改动匹配；`judge.enabled` 已在 `.state.yaml`；无 frontend 域故无 UX BDD 是否成立。
10. **⑤-b/⑤-d 只要求成文**：核对没有 BDD 断言"机制已生效"；⑤-d 的 BDD-49（与既有 `project_phase: bootstrap`/`P2-skeleton.md` 的"骨架声明"消歧）是否合理、是否越出 P0-brief 范围（消歧成文可接受，新增机制不可）。

评审结论须引用具体 BDD 编号锚点（`BDD-NN`）；`agent` 字段写 `requirements-review`（≠ main）。只审不写——不要修改 P1-requirements.md。

### 上游关联

- P0-brief.md：范围权威来源（含末尾「P0 自检记录」及 P0_STALE 补记）
- 设计文档 `docs/design-notes/design-mvwu-protocol.md`（v2.1）：§3.1 / §5.1.1 / §5.1.2 / §7 / §11

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P1-requirements.md（评审对象）
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P0-brief.md（范围基线）
- docs/design-notes/design-mvwu-protocol.md（§5.1.1 / §7.3 核对 SUGGEST-2）
- HANDOFF-TAG0036.md（验证命令口径来源）
- agate/scripts/check-gate.py（`_gate_p2_dispatch_plan`，核对不拒未知键）
</dispatch_guide>
...
- `git diff --stat HEAD~0` 暂存区仅含 `.state.yaml`(phase=P1)、`gate-events.jsonl`、P0-brief 补记、P1 派发文件与 `P1-requirements.md`（尚未提交）——**P1 产出无任何 `agate/` 代码/协议文件改动**，risk_level/phases 声明只能对照"将要发生的改动"评审。
- P1-requirements.md 505 行，`grep -c '^#### BDD-'` = 58；`check-frontmatter.py` 已通过（exit 0）；含 `[NO_NEED_CONFIRM]`。
- 环境：python 3.12.3 / pytest 9.0.3；CI 基线 1666 passed / 2 skipped。
</objective_info>
