...
基于已 approved 的 `P1-requirements.md`（71 条 BDD，含口径 A-H 与修订记录）产出 `P2-design.md`：候选方案 ≥2 + 权衡 + 选择理由；先做**影响面梳理**（改什么 / 不改什么 / 风险在哪，落到"哪个文件的哪个小节"并标 BDD 编号）；四字段 frontmatter（`candidate_count`/`packages`/`domains`/`ui_affected`）+ 正文 `gate_commands` / `files_to_read` / `env_constraints` / `minimal_validation`；并为**本任务自身的 P4 批次**声明 `dispatch_plan`（见约束 6）。

### 约束

1. **P1 的口径是既定契约**：口径 A-H（reason 词表 / 判定顺序 / 列表编码 / `|` 转义 / git 查找范围 / 首词语义 / `git_head` 格式 / …）、exit code 约定、`--observe` 七列、四态 verdict 判定条件均已由 P1 + 评审锁定——**P2 设计它们的落地方式，不得改口径**；如认为某口径不可实现，写 `[NEED_CONFIRM]` 由主 Agent 回 P1 处理，不要在 P2 悄悄改。
2. **P2 要回答的真设计问题**（候选方案的主轴，供参考，可增删）：
   - `check-mvwu.py` 的结构：单文件自包含 vs 拆出与 `agate_common.py` 的共享点（**注意 `agate_common.py` 在零内核改动清单内，不得修改**——只能 import 使用）；如何读 P2 frontmatter（复用 `agate-md-field-get.py` 的 JSON_FIELDS 机制 vs 直接解析）；如何解析 `P4-evidence` 的逐行 `key: value` 日志与 flow 列表（口径 C）；`tests_filter` 首词解析（口径 F）不得执行命令（BDD-19 哨兵文件法）。
   - git 区间（口径 E）的实现调用面：`git merge-base` / `git log` / `git diff --name-only` 的调用与降级（`baseline` 诊断关键字）；平台无关（无 `/tmp` 字面量、显式 utf-8、解释器用 `sys.executable`、Python 3.8+ 语法：禁 `match` / `str.removeprefix`）。
   - ⑤ 组 5 个子项各改**哪个文件的哪一节**（`CONTEXT.md` 追加 5 行三列；`role-system.md` 审查锚点一节；`architect.md` + `P2-design.md` 卡片的三判据 + `tests_filter` 写法；`adr.md` 复审触发条件；P2/P7 卡片的 `decisions/` 落点）。每处给出**插入位置 + 字面标记**（P1 BDD-6/7/9/10/60-66 已列字面锚点，须一一覆盖）。
   - `tests_filter` 写法说明的落点：`P2-design.md` 卡片「dispatch_plan 机器字段」节附近 + `architect.md`「批次设计」节（P1 BDD-7 要求四条既有"硬规则"逐字不变）。
3. **零协议内核改动**（P1 BDD-3/5/12/67 的空 diff 断言清单）：`check-gate.py` / `phases.yaml` / `rules/schema/` / hook 三件套 / `agate_common.py` / `check-state-*` / `check-events.py` / provenance / judge 校验等**一律不改**；`P4-evidence` 不进白名单/目录登记。设计里若出现需要改这些的方案，直接淘汰并在候选方案权衡里写明。
4. **新脚本登记面**（P1 §4.3）：`scripts/README.md`、`tests/README.md`（计数）、`CHANGELOG.md [Unreleased]`、`CONTEXT.md`、P4 卡片等——P2 须列出每处的**改动点与关联 BDD**；`UPGRADING.md`、`agate-summary.py`、CI 清单**不改**（P1 已判不处理）。`check-mvwu.py` 须先于/同批于任何引用它的文档落库（CHECK 10，P1 隐含需求 8）——P2 的批次顺序须体现。
5. **影响面梳理要有客观证据**（grep/rg 命中、读过的消费方代码），不凭印象。`dispatch_plan` 消费方 13 文件的处置沿用 P1 §4.2，不必重扫，但落点要落实到小节。
6. **本任务自身的 `dispatch_plan`**：本任务 P4 是多批任务（至少：脚本+单测 / 卡片+角色文档 / 术语·审查锚点·复审文档 / 登记面）。请声明 `dispatch_plan`（单行 flow YAML；`mode`、`batches[].id` 用**能力名**而非层名——按 P1 判据二 Vertical Slice；每批 `complexity`）。**是否在本任务自身批里写 `tests_filter`（dogfooding）由你按 P1 BDD 与"不主动造样本/自己验证自己偏差"的约束判断并说明理由**；是否逐批 commit **属用户裁决项（P0-brief 决策 A/B），P2 不得替用户选**——若你的批次设计依赖某种 commit 形态，写成 `[SUGGEST: 推荐 X，理由 Y]`（格式严格 `[SUGGEST: ...]`，不要写 `[SUGGEST-N...]`）交主 Agent。
7. **`gate_commands`**（P5/P6 按此执行，P2 后固化）：每条校验拆独立 key、禁 `&&` 串、禁 `P3_xxx` 键。至少含：`P3`（pytest 测试运行器，指向 `agate/tests/unit/` 下新增文件，用 `python -m pytest` 而非裸 python3）、`P5`（全量 CI 口径 `python3 -m pytest agate/tests/ --reruns 1 -n auto`，声明 `P5_timeout_seconds`）、`P5_consistency`（**worktree 自己的** `python3 agate/scripts/check-protocol-consistency.py --strict-errors-only`）、`P5_ruff`（`~/.venvs/agate-dev/bin/ruff check agate/`）、`P5_count`（`bash agate/tests/scripts/count-tests.sh`）。本任务是否含架构适应度维度：按 P1/P0 ⑤-c 判据三自评——若无，写明"本任务无架构适应度检查"。
8. **最小验证（minimal_validation）**：`check-mvwu.py` 涉及 git 与文件解析行为，属"外部系统行为"，须做**最小验证**（例如：在本仓库上跑 `git merge-base HEAD main` / `git log <base>..HEAD --format=%H -- <file>` 确认区间语义；用 20 行脚本验证 flow 列表含 `[a,b-1]` 参数化 id 的解析；确认 `agate-md-field-get.py dispatch_plan` 对含 `|`、引号的 `tests_filter` 透传）——结果写入 `minimal_validation`。验证脚本放 `/tmp` 或 scratchpad，不入库。
9. **files_to_read 控量**：只列 P4 implementer 必需的文件（P4 每批另列），不要把全仓相关文件都列上。
10. **P2 阶段不改 `agate/` 任何文件**（只产出 `P2-design.md`；可选 `P2-progress.md`）。frontmatter 只用 `agate-md-field-set.py`。
11. 产出 `P2-design.md` 后自检：`python3 agate/scripts/check-frontmatter.py <P2-design.md>`；`dispatch_plan` 合法性用 `python3 ~/.agate/scripts/agate-md-field-get.py dispatch_plan <P2-design.md>` 读回核对。

### 上游关联

- `P1-requirements.md`（71 BDD + 口径 A-H + §4 同类扫描 + 修订记录）与 `P1-review.md`（含 SUGGEST-A/B/C 留给 P2/P3 吸收的三条观察：SUGGEST-A 提示 P2 把"批 id 不安全/重复""自相矛盾证据"归口到检查 3/4，勿另立检查函数名/编号；SUGGEST-B/C 为 P3 测试落点）
- `P0-brief.md`；设计文档 `docs/design-notes/design-mvwu-protocol.md`（§4.2 `output` 字段 / §5.1 / §5.1.2 / §7.2）
- `HANDOFF-TAG0036.md`（验证命令、阶段纪律、SELF-GATE 触发面）

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P1-requirements.md
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P1-review.md
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P0-brief.md
- docs/design-notes/design-mvwu-protocol.md
- agate/scripts/agate-md-field-get.py、agate/scripts/check-gate.py（仅读 `_gate_p2_dispatch_plan`）
- agate/scripts/agate_common.py（仅读 workspace/git 相关公共函数，**不改**）
- agate/scripts/check-platform-assumptions.py（"未挂 hook 的独立 check 脚本"参照）与其测试文件（测试风格参照）
- agate/assets/execution-roles/architect.md、agate/phase-cards/P2-design.md、agate/phase-cards/P4-implementation.md
- agate/CONTEXT.md、agate/role-system.md、agate/adr.md、agate/phase-cards/P7-consistency.md
- agate/scripts/README.md、agate/tests/README.md、agate/scripts/check-protocol-consistency.py（CHECK 10 一带）
</dispatch_guide>
...
<objective_info>
主 Agent 本轮实测（worktree 根）：
- P1 已提交（`84d90f7`），`P1-review.md` approved（agent=requirements-review）；`.state.yaml` phase=P1，retries[P1] 1 条。**P2 阶段的 phase 推进随 P2 产出 commit 一起（本 dispatch 不改 `.state.yaml`）。**
- `agate/` 下当前无 TAG0036 改动；`agate/scripts/check-mvwu.py` 不存在。
- 环境：python 3.12.3 / pytest 9.0.3；CI 基线 1666 passed / 2 skipped（`count-tests.sh` 计 1668）；ruff 锁 0.16.4（`~/.venvs/agate-dev/bin/ruff`）。
- 本仓库 `git symbolic-ref refs/remotes/origin/HEAD` → `refs/remotes/origin/main`；`git merge-base HEAD main` = `efb113b`；`merge-base..HEAD` 目前为本任务 P0/P1 的 workspace 提交（可用于 minimal_validation）。
- 稳定版协议根 `~/.agate/v0.71.1/agate`（勿改）；编排类脚本一律用 `~/.agate/scripts/`。
</objective_info>
