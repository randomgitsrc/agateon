...
> 本次是 P2 **复审（第 2 轮）**：第 1 轮 rejected（B1/B2），architect 已就地修订。你与 architect 是不同 agent，请独立判断。

### 目标

复审修订后的 `P2-design.md`，判定 approved / rejected，**覆盖写** `P2-review.md`（含「第 1 轮 B1/B2/N1-N4 逐条闭合核对表」）。

### 约束

1. **B1 独立实测闭合**：在 `/tmp` 或会话 scratchpad 副本中，按 P2 设计的 M18（`GATE_SCRIPT_EXEMPT` 加 `check-mvwu.py`）放入空 `check-mvwu.py` + 修改后的 `check-protocol-consistency.py` 副本，跑 `test_protocol_alignment_review.py -k sg_6` 与 `check-protocol-consistency.py --strict-errors-only`：`test_sg_6` 须绿、`CHECK9-coverage` 无新增。并核查 architect 新增的 R14（其他 glob 全覆盖的既有测试/检查）实测是否属实、有无遗漏（自己 `grep -rn "check-\*.py\|glob(.check-" agate/tests agate/scripts` 抽查）。**副本用后删除，禁止在 worktree 的 `agate/` 下留文件、禁止 git add。**
2. **B2 独立实测闭合**：`P2_DESIGN=<P2-design.md路径> python3 ~/.agate/scripts/agate-read-p5-commands.py`（环境变量 P2_DESIGN 传路径）读回全部 key，逐个 `shlex.split`；核对 `P5_roles_diff` / `P5_history_untouched` 语义保持（roles_diff 仍排除 architect.md；history pathspec 无引号时经 bash 通配的行为在"有匹配 / 无匹配"两种情形下 git 是否等价）。
3. **M18 范围合规**：主 Agent 已批准并在 P1 落 `[BASELINE_CHANGE]`。核对 P2 是否只改 `GATE_SCRIPT_EXEMPT` 一行、未动锚点表；`P5_kernel_diff(_wt)` 清单不含 `check-protocol-consistency.py`；B1 批文件数 ≤3；BDD-70 落点含 M18。
4. **N1-N4 处置**与批 id 改名（`architect-batch-guidance`）是否同步到 frontmatter `dispatch_plan`（用 `FILE=<P2-design.md> python3 ~/.agate/scripts/agate-md-field-get.py dispatch_plan` 读回）+ 正文全部引用（grep 旧 id `tests-filter-authoring-guidance` 应 0 命中，"修订记录"里的旧名说明除外）。
5. **新引入问题排查**：修订是否破坏了第 1 轮评审判"通过"的部分（BDD 落点、零内核、口径未改、批次波次依赖）；§8 是否已无未决 `[SUGGEST` / NEED_CONFIRM。
6. 只对**实质缺陷**打回；第 1 轮已判非阻塞的观察项不重复挑剔（P2 retry 预算 MAX=3，本轮为复审 #1）。`agent: plan-eng-review`（≠ main）；只审不写 P2-design.md、不 git add/commit。

### 上游关联

- 第 1 轮 review 副本（只读）：`/tmp/claude-1000/-home-kity-oclab-agateon--worktrees-agate-TAG0036/88f8db0c-958d-41bc-bbad-e0c8225f4971/scratchpad/P2-review-round1.md`
- `P1-requirements.md`（BDD-70 `[BASELINE_CHANGE]`）；`P0-brief.md`

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P2-design.md
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P2-review.md（第 1 轮，将被覆盖）
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P1-requirements.md
- agate/scripts/check-protocol-consistency.py、agate/tests/integration/test_protocol_alignment_review.py
</dispatch_guide>
...
<objective_info>
主 Agent 机械核对（worktree 根）：`agate/` 下无改动；`check-frontmatter.py` exit 0；`dispatch_plan` 读回合法（含改名后的 `architect-batch-guidance`）；`agate-read-p5-commands.py` 读回 10 个命令、全部 `shlex.split` 通过；P2-design.md 449 行；无 `[NEED_CONFIRM]` 标记。
</objective_info>
