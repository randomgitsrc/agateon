...
> 本次是 P2 **重试 #2**（复审结论 rejected，仅剩 1 个阻塞问题 B3；共享 P2 retry 预算 MAX=3，本次用第 2 次）。**就地做最小修订**，其余（B1/B2/N1-N4 已被复审确认闭合）一字不动。

### 目标

修复 `P2-review.md` 的 **B3**：`P5_history_untouched` 去引号后，bash 在执行前对通配展开，只匹配到仍存在于工作树的目录，**漏检"删除历史任务目录"**（复审实测：设计写法 rc=0 误判、通配转义写法 rc=1 正确拦截）。

### 约束

1. 先读 `P2-review.md` 的 B3 节与「锁定决策」。按复审建议，把 `P5_history_untouched` 的值改为反斜杠转义通配元字符、末 token 不以引号结尾的形式：
   `git diff --exit-code main...HEAD -- agate-workspace/tasks/TAG00\[0-2\]\* agate-workspace/tasks/TAG003\[0-5\]\*`
   （`agate-read-p5-commands.py` 读回后 `shlex.split` 应得 `agate-workspace/tasks/TAG00[0-2]*` 与 `agate-workspace/tasks/TAG003[0-5]*` 两个字面 pathspec，由 git 自己解释 glob）。
2. **自证（必须实测，结果写入 §9 minimal_validation 的 V7 或新增 V 项）**：改后用 `P2_DESIGN=<P2-design.md> python3 ~/.agate/scripts/agate-read-p5-commands.py` 读回 + `shlex.split`；并在 `/tmp` 或会话 scratchpad 的临时 git 仓库（含 `TAG0010-x`/`TAG0011-x`/`TAG0031-x`/`TAG0032-x`，分支上**删除**其中两个）里执行读回后的命令：删除 / 修改 / 新增历史目录均须 rc=1，改 `TAG0036`/`TAG0037` 须 rc=0。临时仓库用后删除，不入库。
3. 同步文字：§7「引号约束」类说明、§14（或修订记录）B2 行"pathspec 语义保持"的表述——如实注明第 1 轮的"去引号、有无匹配两路等价"对**删除**不成立，已被 B3 纠正；§13 B2 守护用例（给 P3）追加一条"删除历史目录须 rc=1"的断言。
4. **修订记录追加「retry #2」小节**：B3 处置 + 涉及章节。其余章节不动；不改 frontmatter（`status` 保持 draft）；不改 `agate/` 下任何文件；不 git add/commit。
5. 完成后自检：`python3 agate/scripts/check-frontmatter.py <P2-design.md>`；`agate-read-p5-commands.py` 读回全部 key 均 `shlex.split` 通过；`FILE=<P2-design.md> python3 ~/.agate/scripts/agate-md-field-get.py dispatch_plan` 仍合法。

### 上游关联

- `P2-review.md`（rejected，B3）；第 2 轮复审副本（只读）：`/tmp/claude-1000/-home-kity-oclab-agateon--worktrees-agate-TAG0036/88f8db0c-958d-41bc-bbad-e0c8225f4971/scratchpad/P2-review-round2.md`

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P2-review.md
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P2-design.md（修订对象）
- agate/scripts/agate-read-p5-commands.py（仅读；在 ~/.agate/scripts/ 有稳定版）
</dispatch_guide>
...
<objective_info>
复审已核实 B1（GATE_SCRIPT_EXEMPT 一行，test_sg_6 绿）、B2（引号）、N1-N4 闭合；仅 B3 未过。`agate/` 下无改动。
</objective_info>
