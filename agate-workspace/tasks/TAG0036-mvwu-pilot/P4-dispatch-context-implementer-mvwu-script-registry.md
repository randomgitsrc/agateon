...
> 本次是 P4 首次派发，**波 3 / 批 `mvwu-script-registry`**（波 3 两批并行，**并发上限 ≤3**；两批文件面互不重叠，只改本批文件）。波 1、波 2 已完成。

### 目标

实现批 `mvwu-script-registry`：新脚本登记面同步——`agate/scripts/README.md` 脚本表、`agate/tests/README.md` 测试映射表（用例数 = 实测）、`CHANGELOG.md` `[Unreleased]`。
...
2. **只改本批文件**：`agate/scripts/README.md`（M13）、`agate/tests/README.md`（M14）、`CHANGELOG.md`（仓库根，M15）——3 文件。**不改**其他任何文件（波 1/2 已落库的脚本与文档都不再动）；不新增 `.sh`；不 git add/commit。
3. **改动落点**（P2 §0.1，插入位置以标题/表格位置定位）：
   - **M13** `scripts/README.md`：脚本表 `check-debt.py` 行之后追加 `check-mvwu.py` 一行，说明含「观测、不阻断、exit 0=任一 verdict、2=用法/目标错误」并含 `--observe`（BDD-69 字面以 P1 为准；用法与 `check-mvwu.py --help` 一致）
   - **M14** `tests/README.md`：测试映射表 `check-platform-assumptions.py` 行之后追加新测试文件行（`test_check_mvwu.py` 行，用例数 = `python3 -m pytest agate/tests/unit/test_check_mvwu.py --collect-only -q` 的**实际数**；若 P1 BDD-69 要求同时登记文档断言测试文件 `test_mvwu_protocol_docs.py`，按 P1/测试 `test_bdd_69_tests_readme_row_matches_collected_count` 的断言登记；README 中总用例数/基线数字若有相应描述须同步，使 `count-tests.sh` 总数 = 1668 + 本任务新增用例数）
   - **M15** `CHANGELOG.md`：`## [Unreleased]`：替换占位句「（暂无——…）」为 `### 新增（TAG0036：…）` 小节，正文含 `TAG0036`，概述本任务交付（`check-mvwu.py` 观测器 / `batches[].tests_filter` 可选键 / `P4-evidence` 落点 / ⑤ 组方法学概念成文），**不写"机制已生效"**；`agate/UPGRADING.md` **不改**
4. **本批 BDD**：BDD-69——逐条读 P1 原文（Given/When/Then 与字面锚点），内容须满足其 Then。
...
6. **计数一致性**：先跑 `bash agate/tests/scripts/count-tests.sh` 拿当前总数，再核对 README 描述与 P1 BDD-69 Then（总数 ≥ 1668 + 新增用例数）；用例数**以实测为准，不凭估算**。
7. **验证（自查，非 gate）**：`timeout 240 python3 -m pytest agate/tests/unit/test_mvwu_protocol_docs.py -q --tb=short -p no:cacheprovider`，**本批相关用例（BDD-69）须转绿**（`test_bdd_71_*` 属 P4 收口的对齐审查产物，不在本批）；`timeout 120 python3 agate/scripts/check-protocol-consistency.py --strict-errors-only` 0 ERROR 且 `CHECK9-coverage` 无新增；`git diff --stat` 确认只动了本批文件。自查通过 ≠ P5 gate 通过，返回里不得声称"P5 已过"。
8. **产出记录**：写 `P4-implementation-mvwu-script-registry.md`（frontmatter **必须用 `agate-md-field-set.py`**：phase=P4 / task_id=TAG0036 / parent=P3-test-cases.md / trace_id=TAG0036-P4-20260919 / type=implementation / created=2026-09-19 / status=draft / `implementation_dir: agate/`；`agent: implementer` set 不接受则 Edit 单行），正文：本批摘要、自查结果、新增文件核对表（无新增文件写"无新增文件"）。主 `P4-implementation.md` 不改。
9. 范围外需求标 `[SCOPE+]`（行首声明格式）写入 `P4-progress.md`（`>>` 追加，行首加 `[mvwu-script-registry]` 前缀）并报告，不直接做。

### 上游关联

- `P2-design.md`（§0.1 本批行 / §0.3 风险 / §6 批表 / §10 files_to_read）；`P1-requirements.md`（BDD-69）；`P3-test-cases-docs.md`

### 输入文件（files_to_read）

- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P2-design.md（§0.1 本批行、§6）
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P1-requirements.md（BDD-69）
- agate/tests/unit/test_mvwu_protocol_docs.py（本批相关用例，只读）
- agate/scripts/README.md、agate/tests/README.md、CHANGELOG.md（本批三文件；只读需要的表格段）
- agate/scripts/check-mvwu.py（仅读 docstring/--help，登记文字须与之一致）、agate/tests/scripts/count-tests.sh（只读用法）
</dispatch_guide>
