...
> 本次是 P6 首次派发（非重试），**P6 验收模式**。P6 产出将由下游独立 judge（P6.5）复核：judge 只读 `P6-evidence/` 与 git log、不读 P6-acceptance.md，故**证据文件必须自足**（每个证据文件写明：对应 BDD 编号、所执行命令原文、命令输出/断言结果、结论）。

### 目标

把 `P1-requirements.md` 的 **全部 71 条 BDD** 逐条**实跑**并取证，产出 `P6-acceptance.md` 与 `P6-evidence/`。每条 BDD 只允许「通过」或「失败」二值；结果必须基于实跑得到的客观输出，**如实记录，不预判、不"修复后再判"**；不允许"调整/跳过/覆盖/deferred"等中间态。

### 约束

1. **只验收不修改**：不修改任何代码/文档/测试（含 `agate/`、`CHANGELOG.md`、`tech-debt.md`）；发现失败 → 如实标失败、写清最小复现，由主 Agent 决定回 P4；不 git add/commit；临时文件放 `mktemp -d`，用后删除。P6 是 self-authored gate：`P6-evidence/` 之外**不得产生任何新文件**（`P6-acceptance.md`、`P6-progress.md` 除外）。
2. **取证方式（每条 BDD 至少一份证据，多条可共享同一证据文件但必须写明各 BDD 的对应段落）**：
   - **行为类 BDD（P3 已把 BDD 落成用例；映射见 `P3-test-cases.md` 与 `P3-test-cases-docs.md`）**：用 `python3 -m pytest <测试文件> -v -p no:cacheprovider` 实跑并把**逐用例 PASSED/FAILED 输出**存为证据（`P6-evidence/pytest-check-mvwu-verbose.log`、`P6-evidence/pytest-mvwu-docs-verbose.log`），验收段落用「BDD-NN → 用例名 → 结果」逐条对应；**但不得只信 pytest 绿**：对以下 BDD 另做**独立于测试代码的直接实证**（构造 `tmp` 任务目录 / 临时 git 仓库，直接运行 `check-mvwu.py` 并存输入与输出）：BDD-19（哨兵：`command`/`tests_filter` 含 `touch <sentinel>`，运行后哨兵文件不存在）、BDD-22/44（含空格 / `|` / 反引号的批 id 与 tests_filter 的一行 / 7 列表现）、BDD-30/31（参数化 id `test_x[a,b-1]` 的 EXPECTED_RED / 近似 id 判 FAIL）、BDD-34（多故障 reason 优先级）、BDD-36/37（exit code：四态均 0、无参数/目录不存在均 2）、BDD-47/48/50（git 区间：merge-base 区间外提交不可见 / 合并提交 boundary / 基线不可确定降级）、BDD-45 修复后的超长耗时（5000 位 `9`）不吞行。这些直接实证各存一个 `P6-evidence/direct-*.log`（含所建输入的关键片段 + 命令 + 输出）。
   - **文档类 BDD（6-12、59-66、68①）**：除 pytest 逐用例结果外，对每个 BDD 用 `grep -n` / `sed -n` 摘录目标文档中**满足该 BDD Then 的原文行（含文件:行号）**存为 `P6-evidence/docs-*.log`；负向条目（无"已生效"字样、`tests_filter: "…python3…"` 0 命中、既有四条硬规则/29 条术语/既有 ADR 逐字未变）用 `git diff efb113b -- <文件>` 或 `grep -c` 实证并存输出。
   - **零内核/范围类 BDD（3、5 的"不修"部分、12、54、55、67 及 M18 相关）**：**在 P6 阶段重新执行**（不引用 P5 结果）`P2-design.md` §7 的 `P5_kernel_diff` / `P5_kernel_diff_wt` / `P5_roles_diff` / `P5_history_untouched` 四条命令（用 `P2_DESIGN=<P2路径> python3 ~/.agate/scripts/agate-read-p5-commands.py` 读回后逐字执行），存 `P6-evidence/diff-*.log`（含命令原文 + `EXIT_CODE: N`）；BDD-68②「不改历史任务产物」同证。
   - **BDD-56（判据 6，真实任务目录）**：在 worktree 根运行 `python3 agate/scripts/check-mvwu.py --observe agate-workspace/tasks/TAG0035-gate-robustness`（再取 `TAG0034-dispatch-routing` 一份亦可），记录 exit code、stdout 各行（应满足 BDD-43 的 7 列）、运行前后 `git status --porcelain` 快照（前后无变化），存 `P6-evidence/bdd-56-observe-real-task.log`——这是**判据 6 的真实证据**，摘录整行输出。
   - **BDD-59/65 的 diff 类断言**（CONTEXT.md 恰新增 5 行且既有 29 行不变；`adr.md` 既有 ADR 正文零删除行）：`git diff efb113b -- agate/CONTEXT.md agate/adr.md` 的统计与逐行摘录存证据。
   - **BDD-69/70/71**：`count-tests.sh` 输出（总数 1844+ 新增的 2 个用例 = 以实测为准）、全量 pytest（CI 口径，`--reruns 1 -n auto`，存 `P6-evidence/full-suite.log`，**文件尾行须为 `EXIT_CODE: <n>`**）、consistency（`--strict-errors-only`，0 ERROR）、ruff、`ls -l` 证明 `P4-protocol-alignment-review.md`（agent≠main、含结论）存在并摘录其 frontmatter 与结论行。
3. **P6-acceptance.md 格式（必须严格）**：frontmatter 用 `agate-md-field-set.py`（phase=P6 / task_id=TAG0036 / type=acceptance / parent=P5-verification.md / trace_id=TAG0036-P6-20260919 / status=draft / created=2026-09-19 / `agent: verifier`（set 不接受则 Edit 单行）/ `pass: N` / `fail: M` / `ui_affected: false`；pass+fail 须 = 71）。正文每条 BDD 一行，行首格式**逐字**：`- PASS BDD-NN: {人话描述} ({证据路径})` 或 `- FAIL BDD-NN: {描述} ({证据路径})`——证据路径写 `P6-evidence/<文件名>`（多个用逗号分隔；每个引用文件必须真实存在且内容实质，不接受 1 行文本充数）。**总结行不得以 `- PASS`/`- FAIL` 开头**（用 `**Summary**: X/71 PASS, Y FAIL`）。验收完成后主 Agent 会跑 `check-p6-format.py --fix`，你无需运行。
4. **无 UI**（`ui_affected: false`，无 frontend 域）：不派 vision-analyst，无截图；本任务没有"人工体验路径"BDD。
5. **post-test 环境残留检查（强制）**：验收动作前后各记录 `git status --porcelain` 快照与 `mktemp` 目录清理情况，写入 `P6-evidence/post-test-residue.log`（前后仅 P6-evidence/、P6-acceptance.md、P6-progress.md 新增；tmp 目录已删除）。
6. **⑤-b/⑤-d 只验「成文到位」**：BDD-60/62 只断言目标文件存在指定节且含字面要点，**不得**在验收中断言"机制已生效/已验证有效"。
7. 每条 BDD 的**关键命令执行前后**往 `P6-progress.md` 用 `>>` 追加一行（命令挂死时便于定位）；所有 bash 加 shell 层 `timeout`（全量 pytest 用 `timeout 900`）。
8. 不得在返回中声称"验收已通过"或"全部 BDD 通过"——只返回路径 + 摘要（数字如实：pass/fail 各多少）。

### 上游关联

- `P1-requirements.md`（71 BDD 原文与口径 A-H，**权威**）；`P3-test-cases.md` / `P3-test-cases-docs.md`（BDD→用例映射）；`P2-design.md` §7（gate_commands）、§12（完成标志）；`P5-test-results/unit.md`（P5 全量结果，背景）；`P4-review.md` / `P4-protocol-alignment-review.md`（背景）

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P1-requirements.md（71 BDD）
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P3-test-cases.md、P3-test-cases-docs.md
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P2-design.md（§7、§12）
- agate/scripts/check-mvwu.py、agate/tests/unit/test_check_mvwu.py、agate/tests/unit/test_mvwu_protocol_docs.py（只读/只运行）
</dispatch_guide>
...
<objective_info>
- 代码与文档 P4 已提交（`4ee499f`），P5 已提交（`e1ac29e`；`.state.yaml p5_pass_commit=4ee499f…`）；P5 到 P6 之间无非产出文件改动。
- 环境：python 3.12.3 / pytest 9.0.3；本地 `main` = `efb113b`（git 基线 `<内核基线>`）；`agate/scripts/check-mvwu.py` 476+ 行；`test_check_mvwu.py` 111 用例、`test_mvwu_protocol_docs.py` 65 用例。
- 真实任务目录候选（BDD-56）：`agate-workspace/tasks/TAG0035-gate-robustness/`、`agate-workspace/tasks/TAG0034-dispatch-routing/`（均为历史已完成任务，只读使用，不得修改其任何文件）。
</objective_info>
