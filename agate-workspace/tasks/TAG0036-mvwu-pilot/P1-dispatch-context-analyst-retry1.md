...
> 本次是 P1 **重试 #1**（requirements-review 结论 needs-revision，共享 P1 retry 预算 MAX=3）。在原 `P1-requirements.md` 上**就地修订**，不重写；首次派发的约束（`P1-dispatch-context-analyst.md` 的 15 条）继续有效。

### 目标

按 `P1-review.md` 修订 `P1-requirements.md`：**F-1..F-5 必改（MUST）**，**F-6..F-9 一并处理（SHOULD，除非有明确理由不处理，须在正文「修订记录」逐条写明处置）**。修订后 BDD 编号仍须 `#### BDD-NN:` 连续不跳号，每条仍只含一组 Given-When-Then。

### 约束

1. **先读 `P1-review.md`（全文）与现有 `P1-requirements.md`**。逐条对照 F-1…F-9 修订，不得顺手改评审判"通过"的 BDD 语义。
2. **SUGGEST-1/2/3 主 Agent 与 reviewer 均同意采纳**：把三条从 `[SUGGEST-*]` 转为已采纳需求（标注"已采纳"）；BDD-9 去掉标题中的条件性表述（F-7）。
3. **F-1**：在隐含需求 5 / 原 SUGGEST-2 处，如实援引设计文档 §5.1.1 权威表中 `UNKNOWN` 触发条件含"边界不可归属"与 §7.3 示例 C 行两处，明示这是**对设计文档 §5.1.1 的有意偏离**（而非"读法不同"）；限定为**阶段 1 观测口径**；声明「verdict 是否纳入 boundary 由阶段 2 与提交粒度决策（P0-brief 决策 A/B，用户裁决）一并复议」；BDD-32 正文保持不变。
4. **F-2**：BDD-33 补充 `tests_filter` 含 `|`（及换行/反引号等特殊字符）时的**输出转义规则**（如 `\|`，你定并给理由），保证七列契约在含管道时仍成立、可直接粘贴进 markdown 表；同规则用于 `batch=<id>` 契约行 id 含不安全字符时的打印形态。可并入 BDD-33 的等价类或新增独立 BDD（则重排编号并同步所有交叉引用，如 BDD-43 的七列断言）。
5. **F-3**：明确 commit 形态/boundary 的 **git 查找范围**（建议：HEAD 相对 `git merge-base HEAD <默认分支>` 的提交区间；无法确定基线时 UNKNOWN + 原因），写进 BDD-36/37/38 的 Given；BDD-39 注明 `agate-workspace/tasks/` 前缀排除的口径（workspace 路径不可配置 / 或按 workspace 根排除，据实测 `agate_common.py` 输出定）。
6. **F-4**：定义 `expected_red` / `failed_tests` 的**列表编码**（如 YAML flow 列表、元素用引号包裹 pytest node id）与**元素相等口径**（建议 node id 精确字符串相等）；补一条含参数化 id（如 `test_x[a,b-1]`）的 BDD 或并入等价类；`expected_red` 值不可解析（非列表）→ verdict UNKNOWN、reason=expected_red，补到 BDD-25 或新增 BDD。
7. **F-5**：拆分 BDD-27（四态 exit 0 vs 用法错误 exit 2）、BDD-29（三种坏输入）；BDD-30 与 BDD-29 前半重叠处合并去冗余。拆/并后**全量重排编号并同步交叉引用**（含 §4 同类扫描、§5、§7、§8 中出现的 BDD 编号）。
8. **F-6**：契约行说明处显式给出**多故障并存的 reason 判定顺序**（建议：tests_filter → evidence → command → exit_code → git_head → expected_red），并用一条 BDD 固化；明确 `git_head` 合法格式（BDD-20）；`id` 重复/非字符串、`exit_code: 0` 但 `failed_tests` 非空（自相矛盾证据）各给一行处置。
9. **F-8**：文档类 BDD（BDD-6/7/9/47-53）每个必含要点至少固定一个**字面标记**（便于 P3 写 grep 断言）；BDD-8 的 `python3` grep 限定为"位于 tests_filter 示例值内"。**F-9**：在 P1 明示"不比对 command 与 tests_filter，属已知观测局限"，并界定检查 2 的"首词"语义（含 `FOO=1 pytest`、`cd x && pytest` 形态的处置）。
10. **`[P0_STALE]` 第 3、4 条**：把夹带的"请主 Agent 补记"指令移出标记外（主 Agent 已补记进 P0-brief，可在正文注明"已补记"）。
11. **正文末尾追加「修订记录（retry #1）」**：F-1…F-9 每条一行：处置（已改/未改+理由）+ 涉及的 BDD 编号（重排后的新编号）。**范围纪律不变**：不得因修订扩张到 P0-brief out-of-scope；如认为必须扩，写 `[NEED_CONFIRM]`。
12. 修订完成后跑 `python3 agate/scripts/check-frontmatter.py <P1-requirements.md>`（worktree 内脚本），并复核 BDD 编号连续（`grep '^#### BDD-' | wc -l` 及序号检查）。frontmatter 字段如需改，只用 `agate-md-field-set.py`。`status` 保持 draft。

### 上游关联

- `P1-review.md`（本次修订依据，含实测记录与 SUGGEST 结论）
- `P0-brief.md`（已含 P0_STALE 补记）；设计文档 `docs/design-notes/design-mvwu-protocol.md` §5.1.1 / §7.3

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P1-review.md
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P1-requirements.md（修订对象）
- {AGATE_WORKSPACE}/tasks/TAG0036-mvwu-pilot/P0-brief.md
- docs/design-notes/design-mvwu-protocol.md
- agate/scripts/agate_common.py（仅读，F-3 workspace 口径）
</dispatch_guide>
...
<objective_info>
- P1-review.md：status=needs-revision；实测抽样核实 15 项中 14 项通过，唯一措辞问题为 `_TASK_FRONTMATTER_FIELDS` 仅存在于 `check-structure-consistency.py:60`（F-7）。
- 当前 BDD 数 58；`count-tests.sh` 基线 1668；`check-frontmatter.py` 对现稿 exit 0。
- retry 计数：P1 第 1 次重试（MAX=3）。
</objective_info>
