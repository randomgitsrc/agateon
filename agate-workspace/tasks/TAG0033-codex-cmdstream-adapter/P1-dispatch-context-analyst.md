...
### 本次是 P1 重试 #1（定向修订，非重写）

`P1-requirements.md` 已由你上一轮产出、已过 requirements-review（`P1-review.md`，`status: needs-revision`）。
本轮**只改 P1-review.md 列出的 3 个修订点，其余一律不动**——不重写正文、不新增 / 删除 BDD、不改已通过的
frontmatter 字段（`domains` / `packages` / `risk_level` / `ceremony` / `phases` 保持不变）、不动 §1-§9 已通过内容。

改完自检：`python3 agate/scripts/check-frontmatter.py agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P1-requirements.md`
仍 exit 0；`[NO_NEED_CONFIRM]` / `[PROD_NOT_TOUCHED]` 保留；BDD 仍 30 条连续。

### 修订点 1〔阻塞〕—— `capability_requirements` `codex-api-key-account` 条目声明与实际不一致

- **位置**：`P1-requirements.md` frontmatter `capability_requirements` 第二条（`need: codex-api-key-account`）+ §10「能力需求声明」对应段。
- **问题**：现写 `status: available` + `available: []`，与同条 `note`（本机 ChatGPT 账号、不具备该能力）
  及仓内先例（TAG0006 `gui-e2e-framework-win` / TAG0009 `真 Windows 执行环境` 均 `status: supplementable` + `gap_note`）矛盾。
- **采用修法 (a)：移除该 `capability_requirements` 条目**（推荐——该 need 本质是「运行环境」问题不是「能力」问题，
  按 P1 卡片「verification_env vs supplementable 边界判断树」口诀「换谁来做都得先有 API-key 账号 → 环境问题」，
  已由 frontmatter `verification_env` + `verification_env_budget` + §7 V8「待有该环境时补（非阻塞）」+ BDD-26 / BDD-29
  正确承载，capability 侧镜像条目冗余且误导）。
  - 删掉 frontmatter `capability_requirements` 列表里的 `codex-api-key-account` 整条（保留第一条
    `codex-cli-installed-authenticated` / `status: available` 不动）。
  - §10 正文：删除 `codex-api-key-account` 那一段，改为一句说明「API-key 账号 model 阵容（§7 V8）是运行环境
    不可得项，走 frontmatter `verification_env` + `verification_env_budget: 止损轮次 2` 声明，非 `capability_requirements`
    条目、非 GAP、非阻塞」。
  - 确认 frontmatter 仍有 `verification_env:` 与 `verification_env_budget:` 两行（本来就有，别删）。
- 若因某原因 (a) 不可行（如 `check-frontmatter.py` 要求 `capability_requirements` 至少 N 条），退用修法 (b)：
  保留条目但 `status: available` → `status: supplementable`，`note` 键名改 `gap_note`，措辞显式写明「仅服务 §7 V8
  非阻塞验证项，非 P1-P4 阻塞项；本机不具备，按 `verification_env` 处理」。**不可维持 `status: available` + `available: []`**。
  在 P1-progress.md 记录为何退用 (b)。

### 修订点 2〔非阻塞，顺手补〕—— §3 同类扫描表补 2 处处置

- 补一行 **S12**：`agate/tests/unit/test_agate_cmdstream_detect.py:371`
  `for platform_word in ("claude", "opencode", "dsh"):`（`test_bdd_24_output_platform_agnostic`）——
  判定「**本次不处理**：负向断言（断言检测输出**不含**这些平台词），加第四键不打破；可选强化：元组加 `"codex"`
  以断言 Codex 也不泄漏」。
- 在 **S10** 处置行补一句：「P4 把 `agate/tests/fixtures/cmdstream/codex-session.jsonl` 加入
  `test_agate_cmdstream_adapters.py::test_bdd_7_fixture_sanitized` 的脱敏校验清单（否则新 fixture 无自动脱敏校验）」。
  若 §6 有对应 BDD（BDD-7 fixture 脱敏 / BDD-19 附近），可在该 BDD 的 Then 里点一句 P4 须同步该清单——不新增 BDD 编号。

### 修订点 3〔非阻塞，建议〕—— BDD-7 Given 明确 P4 兜底 fixture 来源

- 在 `#### BDD-7:` 的 Given 里，把「P2 依实测确定的截断标记形态（见 §4.1.2）」补为
  「P4 先用 §8 `[SUGGEST]` ③ 的『比照 `DSHAdapter._detect_truncated` 保守双信号』兜底 fixture，P5 V4 实测后按
  §4.1.2 收敛」——让 P4 执行者明确不必等 V8 / V4 产出才能写该 BDD 的测试。Given/When/Then 语义不变，只补来源说明。

### 约束

- **P1 基线保护**：这是需求基线的定向修订，改动范围严格限于上述 3 点。不触碰 BDD 的 Given/When/Then 语义
  （BDD-7 仅补 fixture 来源说明，不改判定边界）。
- frontmatter 用 `FILE=agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P1-requirements.md python3 ~/.agate/scripts/agate-md-field-set.py <key> <value>`
  改；`capability_requirements` 这类嵌套结构若 `agate-md-field-set` 不支持逐条删，可直接编辑 frontmatter YAML
  块（这是 analyst 产出文件、允许），改完必跑 `check-frontmatter.py` 确认 exit 0，失败则报告主 Agent。
- 每步落盘 `P1-progress.md`（bash 追加）。
- 不要自己派 requirements-review（主 Agent 负责复审）。

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P1-review.md`（`status: needs-revision`——「必须修订点」节 + 维度 7 是本轮依据）
2. `agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P1-requirements.md`（你要改的文件）
3. `agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P0-brief.md`（核心约束 5 / verification_env 处理惯例）
4. `agate/phase-cards/P1-requirements.md`（「verification_env vs supplementable 边界判断树」节 + 「P1 基线保护」节）
5. `agate/assets/execution-roles/analyst.md`（角色定义）
6. 参考先例：`agate-workspace/tasks/TAG0006-*/P1-requirements.md` 或 `TAG0009-*/P1-requirements.md` 的
   `capability_requirements` 写法（`supplementable` + `gap_note`）——仅在退用修法 (b) 时需要

### 客观查证信息

- `check-frontmatter.py` 对当前 `P1-requirements.md` exit 0（review 维度 8 已核）——`capability_requirements`
  无「至少 N 条」硬性要求（`check-gate.py` 的 `_VISION_STATUSES` 枚举校验只对 frontend/vision 条目生效，
  本任务 `domains: [backend]` 不触发）——所以修法 (a) 移除整条**可行**，优先用 (a)。
- frontmatter 现有 `verification_env: "codex-cli 0.153.4 + ChatGPT 登录（本机已就绪）；API-key 账号环境本机不可得"`
  + `verification_env_budget: "止损轮次 2（独立计数，不占 retries[P5]）；仅适用于 API-key 账号 model 阵容项…"`
  ——这两行本轮**保留不动**。
- §7 表 V8 行、BDD-26、BDD-29 本轮**不改**（review 维度 6 已判定 V8 的 verification_env 侧声明正确）。

### 产出

就地改 `agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P1-requirements.md`（同一文件）。
改完把 `status` 保持 `draft`（复审通过后由主 Agent / 流程推进）。

### 返回

返回：改了哪 3 处（每处一句）+ 修订点 1 用了 (a) 还是 (b)（用 (b) 则说明原因）+ `check-frontmatter.py` exit 码
+ 确认 BDD 仍 30 条、其余内容未动。
</dispatch_guide>
