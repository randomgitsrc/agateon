...
## 你的角色定义
读取并严格遵循：
/home/kity/oclab/agateon/agate/assets/review-roles/plan-eng-review.md

## dispatch-context（核心输入）
读取并严格遵循：agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P2-dispatch-context-plan-eng-review.md
...
## 分阶段落盘（重要，默认启用）
每读完一个输入文件或完成一个关键步骤，立即把发现追加写入 agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P2-progress.md（bash 追加模式）。这样即使你最终无法产出完整报告，progress 文件也能让主 Agent 知道你做了什么。不要等所有文件读完再一次性写——逐条写。
