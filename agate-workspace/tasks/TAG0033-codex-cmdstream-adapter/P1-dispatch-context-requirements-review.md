...
第 1 轮 `P1-review.md` 判 `status: needs-revision`（1 阻塞 + 2 非阻塞）。analyst 已定向修订
`P1-requirements.md`。**本轮只做两件事**：

1. **复核 3 个修订点是否已正确闭合**（下方逐条判据）；
2. **抽查未被意外改动**——第 1 轮已通过的部分（BDD-1..30 除 BDD-7 外的判据、§4 spike 结论、
   §5 范围锁定、frontmatter `domains`/`packages`/`risk_level`/`ceremony`/`phases`、§9 裁剪说明、
   同类扫描 S1-S11、`verification_env` 两行）沿用第 1 轮结论，抽查确认无回归即可，不重新全量评审。

产出：**覆盖第 1 轮 `P1-review.md`**（同一文件，`review_round`/`trace_id` 更新为第 2 轮，
`status` 改 `approved` 或仍 `needs-revision`）。若 approved，正文保留第 1 轮的逐维度结论摘要 +
本轮复核段（3 修订点逐条 CLOSED / NOT-CLOSED + 抽查无回归声明）。

### 修订点复核判据

**修订点 1〔阻塞〕—— `capability_requirements` `codex-api-key-account`**
- analyst 报告用**修法 (a)**：整条移除。
- 判据：(a) frontmatter `capability_requirements` 列表现**只剩** `codex-cli-installed-authenticated` 一条
  （`status: available`，本机确有——不矛盾）；`codex-api-key-account` 条目已不存在。
  (b) frontmatter `verification_env:` + `verification_env_budget:` 两行**仍在**、内容未被削弱。
  (c) §10「能力需求声明」正文对应段已改为说明「API-key 账号 model 阵容是运行环境不可得项，走
  `verification_env` 声明、非 capability 条目、非 GAP、非阻塞」，与 §7 V8 / BDD-26 / BDD-29 指向一致、无残留矛盾。
  (d) `check-frontmatter.py P1-requirements.md` exit 0。
- 全部满足 → 该阻塞项 CLOSED。任一不满足 → 仍 `needs-revision`，写清缺口。

**修订点 2〔非阻塞〕—— §3 同类扫描补 S12 + S10**
- 判据：§3 表新增 **S12** 行（`test_agate_cmdstream_detect.py:371` 的 `("claude","opencode","dsh")`
  负向断言，判定「本次不处理」）；**S10** 处置行含「P4 把 `codex-session.jsonl` 加入
  `test_bdd_7_fixture_sanitized` 脱敏校验清单」一句。
- 独立抽验：`grep -n 'for platform_word' agate/tests/unit/test_agate_cmdstream_detect.py` 确认该负向断言
  确实存在于 :371 附近、确实是「断言输出**不含**平台词」（加第四键不打破）。

**修订点 3〔非阻塞〕—— BDD-7 Given 补兜底 fixture 来源**
- 判据：`#### BDD-7:` 的 Given 现含「P4 用比照 `DSHAdapter._detect_truncated` 保守双信号兜底 fixture、
  P5 V4 实测后按 §4.1.2 收敛」意味的表述；Given/When/Then 的**判定语义未变**（Then 仍是
  `truncated is True` + `output_hash is None`）。

### 抽查无回归清单（确认这些未被本轮修订波及）

- BDD 仍 30 条、编号 1..30 连续（`grep -c '^#### BDD-' <文件>` == 30）
- frontmatter：`domains: [backend]` / `packages: [agate-scripts, agate-docs, agate-tests]` /
  `risk_level: medium` / `ceremony: standard` / `phases: [P1..P8]` 均未变
- `[NO_NEED_CONFIRM]` / `[PROD_NOT_TOUCHED]` 仍在
- §4.1.1 `[P0_STALE: ① 需修正]` 段、§4.6 时效性质疑结论未变
- §5 范围锁定「未发现需超四交付面」结论 + 逃生阀句未变
- §7 V8 行、`verification_env_budget` 未被削弱

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P1-requirements.md`（修订后——受评对象）
2. `agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P1-review.md`（第 1 轮结论——你要在其基础上更新）
3. `agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P1-progress.md`（analyst 本轮修订落盘记录）
4. `agate/assets/review-roles/requirements-review.md`（角色定义）
5. `agate/phase-cards/P1-requirements.md`（P1 gate 规则——复评产出仍须含 BDD 编号锚点 + `status: approved` + `agent != main`）
6. `agate/tests/unit/test_agate_cmdstream_detect.py`（:371 附近——S12 独立抽验）

### 客观查证信息（主 Agent 已核实，2026-09-09）

- analyst 修法 (a) 已落地：frontmatter `capability_requirements` 现只剩 `codex-cli-installed-authenticated`
  一条；`verification_env` / `verification_env_budget` 两行保留；`check-frontmatter.py` exit 0；
  `grep -c '^#### BDD-'` == 30；S12 行已在 §3 表（line ~87）；BDD-7 在 line ~299。
- 主 Agent 已在 `.state.yaml` 记录 `retries.P1 round 1`（failure_mode: quality）。
- P1 gate 规则（`check-gate.py P1`）：P1-review.md 存在 + `status: approved` + `agent != main` +
  含 `#### BDD-NN:` 格式的 BDD 编号锚点 → exit 2。你的产出必须含 BDD 编号引用。

### 产出文件字段

用 `FILE=agate-workspace/tasks/TAG0033-codex-cmdstream-adapter/P1-review.md python3 ~/.agate/scripts/agate-md-field-set.py <key> <value>`
更新：`status`（approved / needs-revision）、`trace_id=TAG0033-P1-review-20260909-r2`、`review_round=2`、
`created=2026-09-09`（其余 frontmatter 字段沿用）。写入失败照错误提示修正，不手写 frontmatter。
</dispatch_guide>
