...
> 本次是 P5 **重跑 #1**：P4 修复批 t17-fixture-fix 已提交（HEAD 895a10c，改动仅 agate/tests/helpers_tag_repo.py），需重新完整验证。工作区应干净（P6-evidence/、P6-dispatch-* 等未跟踪文件属 P6 前置产物，不要动；gate-events.jsonl 可能有 hook 追加）。上一轮 P5 产出（P5-test-results/）由你**覆盖写入**（是你自己上一轮产出的同名文件，允许覆盖，不得删除其他文件）。

### 目标

在 worktree（`/home/kity/oclab/agateon/.worktrees/agate-TAG0037`）执行 `P2-design.md` 的 `gate_commands`（§7）全部 P5 类命令，逐条判定并产出 `P5-test-results/`：

1. `P5`：`python3 -m pytest agate/tests/ --reruns 1 -n auto`（CI 口径；期望 failed=0；基线口径 passed ≥ 1842、skipped ≤ 2 —— 本任务净增测试，passed 应显著大于 1842；**若 skipped > 2 须说明**：BDD-43/45 仅在 `GITHUB_ACTIONS=true` 且缺 CLI 时 skip，本地环境应为 FAIL 或通过，不应 skip）。
2. `P5_consistency`：`python3 agate/scripts/check-protocol-consistency.py --strict-errors-only`（worktree 自己的脚本；0 ERROR）。
3. `P5_ruff`：`~/.venvs/agate-dev/bin/ruff check agate/`。
4. `P5_shellcheck`：`shellcheck -S warning install.sh agate/scripts/*.sh`。
5. `P5_release_build`：真实构建冒烟。**⚠ 输出目录改用 scratchpad 下新建带序号目录**（如 `<scratchpad>/p5-release-1`），不要写 /tmp 固定路径：`python3 agate/scripts/agate-release.py build --tag v0.72.0 --repo . --outdir <scratchpad>/p5-release-1 --notes-out <scratchpad>/p5-release-1.notes.md --skip-offline`，另对产物跑 `sha256sum -c`（在产物目录内）并记录 tarball 成员数、字节数。
6. **额外核对**（P5 通用）：`python3 agate/scripts/agate-install.py --check`（在隔离 `AGATE_HOME=<scratchpad>/p5-home-1` 下，勿动真实 ~/.agate）；`bash agate/tests/scripts/count-tests.sh` 计数与文档一致（无漂移）。
7. **P4 已知待验证项**：`test_t15_real_tree_smoke_pack_install_resolve`（读已提交 HEAD，P4 提交后应转绿）必须通过，明确报告。

### 约束

1. 每条命令加外层 `timeout`（pytest 全量 timeout 580；其余 60–120），命令串行执行（资源密集型不并行）。设 `PYTHONDONTWRITEBYTECODE=1`。
2. **失败处理**：任何失败先判定真失败 vs 环境问题 vs flaky（flaky 记三振），**不要改代码或测试**，不要"让它变绿"；把失败清单与初步归因如实写入产出，主 Agent 判定。
3. **数据安全（强制）**：只读验证 + 仅在 scratchpad/pytest tmp 下写；禁止 `rm -rf`/清空重建/删除任何非本次新建的文件；不触碰真实 `~/.agate`、开发 checkout、git 分支/tag；不 git add/commit/push/tag；不触发真实 CI、不创建 Release。仓库内除 `P5-test-results/` 外不得新增或修改任何文件（`git status` 前后对比并记录）。
4. 产出（均在 `{AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P5-test-results/`）：
   - `unit.md`：全量 pytest 结果，含 test runner 输出签名行（`passed`/`failed` 汇总）、skipped 说明、命令与耗时；标注"预存失败"（若有，与本次改动无关者）；文首标注 `[PROD_NOT_TOUCHED]`。
   - `fail-list.txt`：failed 测试 id 逐行 `FAILED ` 前缀，无失败则空文件。
   - `checks.md`：consistency / ruff / shellcheck / release_build / --check / count-tests / t15 各自命令、exit code、关键输出摘录、判定。
5. 不写行首 `- PASS`/`- FAIL`（用表格或句子）。返回：`P5-test-results/` 路径 + 一句话摘要（≤40 字，含 passed/failed/skipped 数）。

### 上游关联

P2-design.md §7（gate_commands 权威）、P4-implementation.md、P3-test-cases.md §8（裁决记录）

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P2-design.md（§7）
- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P4-implementation.md
</dispatch_guide>
