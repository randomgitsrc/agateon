...
> 本次是 P1 **重试 #1**（requirements-review 返回 needs-revision：4 项 MAJOR，无 BLOCKER）。

### 目标

按 `P1-review.md` 修订 `P1-requirements.md`，使其可通过复审。逐条处理全部 MAJOR（M-1…M-4）与全部 MINOR，每条在正文末尾（或 P1-progress.md）给出"已修改 / 不修改 + 理由"处置记录。

### 约束

1. 先完整读 `P1-review.md`（含 MINOR 节，本派发不重复其细节），以其修改建议为准；你可提出更优方案但须写明理由。
2. **M-1 采用方案 (b)**（主 Agent 决策，编号不漂移）：在 §4 头部加"全或无规则"（含多个 When/夹具的 BDD，全部子场景逐一判定，任一失败则该 BDD 整体 FAIL，P6 须逐子场景留证据），并在评审所列 9 条 BDD 标题后标注 `[参数化]`。
3. **M-2**：解决 BDD-14 与 BDD-20 的矛盾（例如测试 tag 场景下 CHANGELOG 的取段规则/预置方式），并明确测试 tag 上资产名的判定口径；更正 S-9 中不成立的事实依据。
4. **M-3**：BDD-37 白名单须按其自身范围与模式实扫核实（自己跑 grep 验证"落地即绿"，而非估计）。
5. **M-4**：BDD-42/43/45 改为二值可判定；消除 `[CAPABILITY_GAP]` 第三态出口（如改为明确的降级判据），并与"隔离 AGATE_HOME"前提自洽。
6. 范围不得扩大（P0-brief 为准）；用户已裁决三项不变（CI 许可含触发/权限；legacy 仅文档三步 + fail-closed 提示；v0.73.0 minor bump）。
7. 不改 BDD 编号（不新增插入导致漂移；确需新增则追加在末尾）。修订后重跑 `check-frontmatter.py` 与 `check-routing.py`，且不得改动 frontmatter 已有字段（`ceremony`/`agent` 保持）。
8. 不 git commit。不动 P1-review.md。返回：路径 + 一句话摘要（≤30 字）。

### 上游关联

- P1-review.md（本次修订依据）、P0-brief.md

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P1-review.md
