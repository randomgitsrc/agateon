...
> 本次是 P2 评审**复审 #1**（architect 已按你与另一评审的意见修订，新增 §14 评审处置表）。

### 目标

复审修订后的 `P2-design.md`，覆盖写 `P2-review-eng.md`（`status: approved | rejected`，`agent: plan-eng-review`）。

### 约束

1. 逐条核对你首轮的所有发现是否被**真实**解决（读设计正文与必要时实测，不只看 §14 处置表自述）；BLOCKER/HIGH/MAJOR/MEDIUM 未解决则 rejected。
2. 检查修订本身是否**新引入**问题（含另一评审角色的修订对你领域的影响），只报实质问题；无 BLOCKER/HIGH/MAJOR 则 approved，避免无谓打回。
3. 已裁决项不再异议（S-1…S-19、C-1、install.sh 不 git pull、CI workflow 授权）。
4. **数据安全（强制）**：实验只在 scratchpad 下新建带序号目录；禁止 `rm -rf`/清空重建/删除任何非本次新建文件；不触碰真实 ~/.agate、开发 checkout、git 分支/tag；不 git commit；仓库内除输出文件外不新增文件；返回时列出你新建的 scratchpad 目录。
5. 不用行首 `- PASS` / `- FAIL`。返回：输出路径 + 一句话摘要（≤30 字，含最高严重级别）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P2-design.md
- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P2-review-eng.md（首轮，待覆盖）
