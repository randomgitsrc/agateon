...
> 本次是 P1 评审**复审 #1**（首轮 needs-revision：M-1…M-4 + 11 条 MINOR；analyst 已修订，BDD 现为 52 条）。

### 目标

复审修订后的 `P1-requirements.md`，覆盖写 `P1-review.md`（frontmatter `status: approved | needs-revision`，`agent: requirements-review`）。

### 约束

1. 先读旧 `P1-review.md` 的发现清单，逐条核对修订是否真正解决（不只看 analyst 自述的处置记录）：M-1（§4 全或无规则 + 9 条 `[参数化]` 标注）、M-2（BDD-14/BDD-20 矛盾与资产名口径、S-9 事实依据）、M-3（BDD-37 白名单——**自行跑 grep 实扫验证落地即绿**）、M-4（BDD-42/43/45 二值可判定、无 `[CAPABILITY_GAP]` 第三态、与隔离 AGATE_HOME 自洽）、全部 MINOR。
2. 检查新增 BDD-51/52（编号在末尾追加）是否可二值判定、是否越出 P0-brief 范围，以及与既有 BDD 无新矛盾。
3. 判据 1–11 仍全部有 BDD 覆盖；BDD 编号连续；frontmatter 未被破坏。
4. 新发现只报此次修订**新引入**或首轮遗漏的实质问题，避免无谓打回；无 BLOCKER/MAJOR 则应 approved。
5. 评审意见中不使用行首 `- PASS` / `- FAIL` 格式。不改 P1-requirements.md，不 git commit。返回：路径 + 一句话摘要（≤30 字）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P1-requirements.md
- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P1-review.md（首轮，待覆盖）
