...
> 你是本次 P4 评审的**专家组组长**（角色：review，汇总），不是新的评审员。

### 目标

汇总 `P4-review-code.md`（review）与 `P4-review-cso.md`（cso）两份评审，产出 `P4-review.md`（统一 `status: approved | rejected`，`agent: review`）。

### 约束

1. **不发表新意见，只汇总**：列两位专家最终结论、遗留发现（MINOR/LOW 等）及是否需要后续处置（P5/P6/P7）。
2. 规则：任一专家标 BLOCKER（CRITICAL/HIGH 视为 BLOCKER）→ rejected；专家分歧 → 标"专家组分歧"；全票无 BLOCKER → approved。**核对两份文件 frontmatter `status` 真实值与 `agent` 字段**，不凭本说明。
3. 汇总须包含：两位专家对各批 [DESIGN_GAP]（discard_backup 返回 False+警告 / rollback_swap 语义 / make_work_dir 创建根目录 / pack 输出目录已存在拒绝覆盖）的裁定结论；cso 的删除点清单结论（数据安全）。
4. P4-review.md 的 `agent` 必须是 `review`（非 main）；正文含 BDD 编号引用锚点；不用行首 `- PASS`/`- FAIL`。
5. 数据安全：只读；除输出文件外不改任何仓库文件；不 git add/commit；不删除任何文件。
6. 返回：路径 + 一句话摘要（≤30 字）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P4-review-code.md
- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P4-review-cso.md
</dispatch_guide>
