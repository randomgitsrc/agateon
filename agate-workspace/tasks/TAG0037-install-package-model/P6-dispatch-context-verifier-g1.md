...
> 本次是 P6 验收 **第 1 组（BDD-1…26：契约/旧形态/离线修复/Release/portable/在线安装）**（首次派发）。

### 目标

对 P1 的 **BDD-1 至 BDD-26** 逐条验收，产出 `P6-evidence/g1/` 证据与 `P6-evidence/g1/results.md`（PASS/FAIL 行 + 证据引用）。

### 组特定要求

- **BDD-20（真实 CI 实跑，用户已授权推测试 tag；清理由用户手动做，你不得删除任何 tag/Release）**：测试 tag `v0.73.0-tagtest.1` 已由主 Agent 推送到 origin（**只推远端 tag，本地无同名 tag**），指向提交 `d6dd3cb007a414f7dd5011c4031aa44326335a00`（P5 提交），workflow run id `35505427563`，结论 success，已创建预发布 Release，资产：`agateon-v0.73.0-tagtest.1.tar.gz`、`agateon-v0.73.0-tagtest.1-offline-linux-x86_64.tar.gz`、`agateon-v0.73.0-tagtest.1-offline-windows-x86_64.tar.gz`、`SHA256SUMS`。按 P3 §7.5 BDD-20 行与 P1 BDD-20 的全部 Then 子项核对：`gh -R randomgitsrc/agateon run view 35505427563`（jobs/steps 结论、被连带触发的现有 workflow 用 `gh run list --commit d6dd3cb...` 或按 tag 检索并记录）、`gh -R randomgitsrc/agateon release view v0.73.0-tagtest.1 --json ...`、把资产下载到 scratchpad 新建目录（`gh release download`，只读）→ `sha256sum -c SHA256SUMS`、tarball 成员（本体 asset 与本地 `agate_package`/`agate-release.py` 对同一提交的构建逐成员字节比对：可用 `python3 agate/scripts/agate-release.py build --ref`/等价参数对提交 d6dd3cb 构建到 scratchpad，若脚本只支持 tag 则用 `git ls-tree`+`agate_package.list_package` 求成员集合与 blob 逐字节比对）、offline `manifest.version` 严格 `vX.Y.Z`、prerelease 标注与 notes 首行、资产名 tag 原样、`SHA256SUMS` 局限声明。**"清理后才可跑 CHECK 7"这一子项**：因清理由用户手动做且本地无同名 tag，说明本地 CHECK 7 不受影响并给出 `git tag -l 'v0.73.0*'` 输出为证；远端仍存在的测试 tag/Release 在 results.md 标注"待用户手动清理（列出精确名称）"并判定该子项依 P1 BDD-20 文字（读原文判定，不放宽也不苛求）。如 P1 BDD-20 明确要求由本任务完成清理才算 PASS，则据实判 FAIL 并说明。
- **BDD-17（portable 无 git）**：用上面下载的**真实 Release 本体 tarball** 在无 git 的隔离环境（PATH 仅含 python3+pyyaml+coreutils 的符号链接目录，先断言 `git` 不可用）逐字执行 `agate/UPGRADING.md`「portable 安装」小节的命令（隔离 `AGATE_HOME`），验证 `agate-resolve.py` 解析成功；命令输出留证。
- **BDD-22/23（在线只装本体 + 冗余量化，真实仓库实测）**：在隔离 `AGATE_HOME`（scratchpad 新建）下用 `AGATE_REPO_URL=file://<本 worktree 路径>` 对 tag `v0.72.0`（本地已存在）或 HEAD 做在线安装（`python3 agate/scripts/agate-install.py latest` 或对应子命令，参照 P2 §11/§3.3 与 P3 §7.5），`du -sb` 记录安装版本目录字节数 S 与 `agate_package` 求和 B，判 `S <= 1.25 × B`（v0.72.0：上限 2,331,089 B），并列出版本目录顶层条目、确认无 tests/pyc/__pycache__/agate-workspace 等。
- BDD-9/11/12/16（离线）：证据除 pytest 外，另做一次**真实 pack→bundle 内入口 install-offline→resolve** 的端到端实跑（隔离目录，`pip` 用现成 PATH shim 方式或跳过 wheel 下载的 `--skip-*` 参数，参照 `helpers_tag_repo.py`/P3 T-15；勿联网），留存命令与输出。
- BDD-4（三路径同构）：H3（portable）用例读 UPGRADING 文档命令，须通过；输出留证。

### 通用约束（强制）

1. **依据**：`P1-requirements.md`（BDD 权威，含 [BASELINE_CHANGE] 注记）、`P3-test-cases.md` §8.1（BDD→测试文件总表）、`P2-design.md`、`P5-test-results/`（P5 已通过：2292 passed / 0 failed / 2 skipped，HEAD 467fec7；P6 不复用为 BDD 证据，须自己留存证据）。当前 HEAD 含 P4、P5 提交，工作区应干净。
2. **证据要求（T046 教训：既要功能验证也要 gate 格式）**：每条 BDD 对应至少一个**非空、含实际输出**的证据文件（不接受 1 行文本充数），证据放在 `P6-evidence/{组}/` 下，文件名 `bdd-NN-*.log|md|txt`。证据 = 对该 BDD 映射测试的 `pytest -v` 实跑输出（用例 id + PASSED，参数化 BDD 的**全部子场景**都要 PASSED——P1 §4 全或无规则）加上 BDD 要求的**命令/真实环境实测**（P3 §7.5 列出的非 pytest 验收项）。测试命令：`PYTHONDONTWRITEBYTECODE=1 python3 -m pytest <文件> -k "<表达式>" -v -p no:cacheprovider`；每个 BDD 的 `-k` 用 `bdd_N`/`BDD-N`（或按用例 docstring 检索）精确选择，必须实际选中 ≥1 用例且全部通过，选中 0 个用例视为该 BDD 无证据。
3. **PASS 行**：写入本组 `results.md`，每行格式 `- PASS BDD-NN: {描述} ({证据相对路径}[, ...])`；FAIL 行 `- FAIL BDD-NN: ...`。只允许 PASS/FAIL 两态，**不得出现"待定/跳过/延后/N/A"**；若某 Then 子项无法验证或失败，该 BDD 必须写 FAIL 并写明原因，不得凭推断写 PASS。不要修改任何代码/测试/文档来让它变绿。
4. **数据安全（用户强制）**：只读验证；写入仅限 `{AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P6-evidence/{组}/` 与 scratchpad 新建带序号目录（`/tmp/claude-1000/-home-kity-oclab-agateon--worktrees-agate-TAG0037/9e4aedbc-aa1a-42f7-9d40-57703b1a2c96/scratchpad`）；禁止 `rm -rf`/清空重建/删除任何非本次新建的文件；不触碰真实 `~/.agate`、开发 checkout（/home/kity/oclab/agateon）、git 分支/tag；不 git add/commit/push/tag；**不删除任何 GitHub tag/Release**、不推送任何 ref；所有安装/打包验证使用隔离 `AGATE_HOME`/`--dest-root`（scratchpad 下新建目录）；仓库内除 `P6-evidence/{组}/` 与 `P6-progress.md` 外不得新增或修改任何文件（开始与结束各记一次 `git status --short`，写入 `P6-evidence/{组}/git-status.txt`，内容为两次输出对比）。设 `PYTHONDONTWRITEBYTECODE=1`。
5. 每条命令外层加 `timeout`（单测文件 280、全量 580）；串行执行；失败先判定真失败/环境问题/flaky，如实记录。
6. 不写行首 `- PASS`/`- FAIL` 之外的格式于 results.md 的 BDD 条目之外（总结行用 `**Summary**: n PASS, m FAIL`）。返回：results.md 路径 + 一句话摘要（≤40 字，含 PASS/FAIL 数）；不声称"验收已通过"。

### 上游关联

P1-requirements.md、P3-test-cases.md §8、P5-test-results/、P4-implementation.md
...
- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P2-design.md（§11、§14）
- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P5-test-results/
- 参考 `agate/tests/helpers_tag_repo.py`（构造隔离 pack/install 的写法）
</dispatch_guide>
