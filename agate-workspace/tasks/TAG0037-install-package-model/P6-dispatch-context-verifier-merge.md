...
> 本次是 P6 验收 **汇总（把 g1/g2 两组结果转抄整合为唯一 P6-acceptance.md）**（首次派发）。

### 目标

读 `P6-evidence/g1/results.md` 与 `P6-evidence/g2/results.md`，整合出唯一的 `P6-acceptance.md`（frontmatter 见下）与交叉核对记录；**你不重新验证、不改证据，只转抄整合并核对**。

### 组特定要求

- frontmatter：`phase: P6`、`task_id: TAG0037`、`type: acceptance`、`parent: P5-verification.md`（沿用协议样例值）、`trace_id: TAG0037-P6-20260920`、`status: draft`、`created: '2026-09-20'`、`agent: verifier`（agate-md-field-set 拒绝写 agent 键时，主 Agent 授权你手写该行）、`pass: <int>`、`fail: <int>`、`ui_affected: false`（与 P2 一致）。若合并结果 fail>0，如实写，不要改写成 PASS。
- 正文：① 每条 BDD 一行 `- PASS BDD-NN: 描述 (P6-evidence/gX/xxx)` 或 `- FAIL ...`（从两组 results.md 转抄，证据路径改写为相对任务目录的 `P6-evidence/...`，确认路径真实存在）；② **交叉核对节**：BDD 编号合集 = 1..52 无重复无遗漏（写出核对命令与输出）；③ 总结行用 `**Summary**: n/52 PASS, m FAIL`（不要以 `- PASS`/`- FAIL` 开头）；④ 环境残留检查节：汇总两组 `git-status.txt` 的前后对比与 scratchpad 新建目录清单（post-test 环境残留检查，强制步骤）；⑤ 已知遗留/待用户手动清理事项：远端测试 tag `v0.73.0-tagtest.1` 与其预发布 Release（精确名称，用户已选择手动清理）；BDD-50 的 P8 复验项。
- 转抄后运行 `python3 ~/.agate/scripts/check-p6-format.py --fix agate-workspace/tasks/TAG0037-install-package-model/P6-acceptance.md`（编排类脚本用稳定版）。不要运行 check-gate（由主 Agent 亲自跑）。
...
- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P6-evidence/g1/results.md
- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P6-evidence/g2/results.md
- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P1-requirements.md（核对 52 条编号）
</dispatch_guide>
