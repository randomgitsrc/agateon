...
独立评审 `P2-design.md`（安装与多版本模型统一方案），产出 `P2-review-cso.md`：frontmatter `status: approved | rejected`、`agent: cso`；正文给出分级发现（BLOCKER/MAJOR/MINOR）与可执行修改建议，并标注覆盖的 BDD 编号。只审不写，不改 P2-design.md。

### 评审重点

以供应链 + 安装器安全为主（domains 含 security）：
1. 打包安全：tarball 成员路径穿越/绝对路径/符号链接/硬链接/设备文件（BDD-15 成员安全）、边界规则"默认拒绝"是否可被绕过、确定性打包是否引入信息泄露（uid/gid/mtime）。
2. 安装器：`--adopt`、install-offline 的替换语义（先临时目录后换位）是否有 TOCTOU/半装/回滚缺口；解压他人 tarball 的路径校验。
3. Release workflow：`contents: write` 是唯一权限、触发面（仅 push.tags v*）、tag 名注入到 shell 的风险（`${{ github.ref_name }}` 直插脚本）、不使用 secrets、`gh release create --verify-tag`、无第三方 action；SHA256SUMS 是否真正提供完整性（同源分发的局限需如实标注）。
4. 迁移提示/文档中是否诱导用户执行危险命令（mv/rm 范围）；软链 fail-closed 是否可被绕过（符号链接基址、AGATE_HOME 指向）。
5. 测试 tag 推送与清理流程（§11）的副作用/污染面。
按 STRIDE 矩阵 + 严重级别（CRITICAL/HIGH/MEDIUM/LOW）输出；CRITICAL/HIGH 视为 BLOCKER。

### 通用约束

1. 用户已采纳 P1 §7 全部 SUGGEST（S-1…S-19），CI workflow 新增已获授权——不要对这些"已裁决"项再提异议，只审其**设计实现**是否正确。
2. 主 Agent 已裁决：Release 为 4 资产（3 tarball + SHA256SUMS）按"至少含这 3 个 tarball + SHA256SUMS"解读；upgrade-bootstrap 的 `install.sh git pull` 不做（仅文档化，backlog）。设计文档已按此修订（若你看到旧表述以最新版为准）。
3. 结论须基于实测/实读代码，不凭印象；可在 scratchpad（/tmp/claude-1000/-home-kity-oclab-agateon--worktrees-agate-TAG0037/9e4aedbc-aa1a-42f7-9d40-57703b1a2c96/scratchpad）做实验，不得触碰真实 ~/.agate、不 git commit、不改除输出文件外的任何仓库文件。
4. 不使用行首 `- PASS` / `- FAIL` 格式。
5. 返回：输出路径 + 一句话摘要（≤30 字，含最高严重级别）。
...
- 设计涉及的现有源码：install.sh、agate/scripts/agate-install.py、install-offline.py、agate-pack-offline.py、agate_common.py、agate-resolve.py
</dispatch_guide>
