...
> 本次是 P2 **重试 #1**（plan-eng-review 与 cso 均 rejected：eng 1 BLOCKER + 5 MAJOR + 8 MINOR；cso 1 HIGH + 6 MEDIUM + 7 LOW）。

### 目标

读 `P2-review-eng.md` 与 `P2-review-cso.md`，修订 `P2-design.md`，使两份评审的 BLOCKER/HIGH/MAJOR/MEDIUM 全部真实解决，LOW/MINOR 逐条处置。

### 约束

1. 每条发现在设计文档末尾（新增 §14「评审处置表」）给出：编号 → 已采纳（落点章节）/ 不采纳（理由）。BLOCKER/HIGH/MAJOR/MEDIUM 不得"不采纳"，除非有实测证据反驳（写明证据）。
2. 关键必改项：eng B-1（`__pycache__`：入口 `sys.dont_write_bytecode`、`-B`、拷贝忽略 pyc、`compute_sha256` 免疫、测试以 bundle 内入口执行）；eng M-1（§8 yaml 块必须可 `yaml.safe_load`，改完自己跑 `python3 agate/scripts/check-protocol-consistency.py --strict-errors-only` 确认该文件 0 ERROR）；eng M-2（`--adopt` 用同目录兄弟安装器）；eng M-3（既有测试夹具不含 `agate_package.py` 的处理）；eng M-4/M-5；cso F-1（软链守卫尾斜杠绕过——先实测复现再设计，须覆盖 realpath/尾斜杠/`..`）、F-2…F-7。
3. 已裁决项不变（S-1…S-19 采纳、C-1 四资产解读、install.sh 不 git pull、CI workflow 授权）。范围不扩大。
4. **数据安全与整洁（强制，用户要求）**：任何实验只在 scratchpad 下**新建带序号的目录**（如 `exp-N`），**禁止 `rm -rf`、禁止清空重建、禁止删除任何非本次新建的文件**；不得触碰真实 `~/.agate`、开发 checkout、git 分支/tag；实验结束**不留**垃圾文件（仓库内不得新增除 P2-design.md 以外的文件；scratchpad 实验目录留给主 Agent 统一清理，在返回摘要里列出你建了哪些）。
5. 设计里涉及的任何"测试/验证会执行删除"的步骤（如测试 tag 清理，§11）必须显式标注"需主 Agent 取得用户许可、且仅删除本次创建的 tag/Release"，并给出保护措施（先列出将删项、名称精确匹配 `v0.73.0-tagtest.N`、不使用通配符）；cso F-7 的竞态问题也须在此解决（如先等 workflow 结束再清理；或改为更安全的验证方式）。
6. frontmatter 不动（含 dispatch_plan；若批次依赖因 eng M-4 需调整，可改 dispatch_plan 内容，保持合法：mode/批数 ≤ parallel_limit）。
7. 不 git commit，不改除 P2-design.md 外的仓库文件。返回：路径 + 一句话摘要（≤30 字）+ 列出你新建的 scratchpad 目录。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P2-review-eng.md
- {AGATE_WORKSPACE}/tasks/TAG0037-install-package-model/P2-review-cso.md
