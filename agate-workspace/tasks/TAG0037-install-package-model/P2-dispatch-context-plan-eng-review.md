...
独立评审 `P2-design.md`（安装与多版本模型统一方案），产出 `P2-review-eng.md`：frontmatter `status: approved | rejected`、`agent: plan-eng-review`；正文给出分级发现（BLOCKER/MAJOR/MINOR）与可执行修改建议，并标注覆盖的 BDD 编号。只审不写，不改 P2-design.md。

### 评审重点

1. 方案能否让 P1 的 52 条 BDD 全部落地（§4 映射是否真实、有无遗漏/虚挂）。
2. 排除机制选型（候选 A/B/C 比对是否公正、实测是否可信、选定的自写构建器是否过度设计、`agate_package.py` 与 `agate-release.py` 的接口是否合理）。
3. **兼容红线**：`_protocol_root` 零改动；已装旧形态可解析；探测序；D-6/D-7（替换已存在 vdir、删除 .installed-version/.agate-root）是否会破坏既有安装或既有测试。
4. legacy 删除（D-8）：软链基址走普通链是否真的满足 BDD-27/28/51；有无遗漏调用面。
5. 批次设计 `dispatch_plan`：批间文件冲突（agate_common.py、install.sh、UPGRADING.md 等共享文件）、依赖顺序、复杂度评估是否合理。
6. gate_commands 合法性（P3 只裸键、无 && 串联、超时声明）、files_to_read 是否精炼。
7. 测试策略（供 P3）是否足够抓住 BDD-11 同源假设复发；风险表的缓解措施是否成立。
