...
基于 `P1-requirements.md`（52 条 BDD，approved）产出 `P2-design.md`：安装与多版本模型统一的技术方案——版本目录结构契约（含本体精确边界的**排除机制选型**）、离线安装 pack/install 修复、Release workflow + 本体/offline tarball 构建、在线只装本体、legacy 软链彻底删除。每个 BDD 至少映射到一个设计落点。

### 约束

1. **用户已裁决（不再有疑问，直接采纳）**：P1 §7 全部 `[SUGGEST]`（S-1…S-19）**已被用户采纳**——`agate/tests/` 排除出包；`agate/AGENTS.md` 入包；`LICENSE` `NOTICES.md` `CHANGELOG.md` 作为 `vX.Y.Z/` 顶层已登记文件入包；`README*`/`pyproject.toml`/`SELF-GATE.md`/根 `AGENTS.md`/`CLAUDE.md`/`install.sh`/`HANDOFF-*.md` 排除；边界机制须"默认拒绝"（S-6）；v0.73.0；冗余阈值 S≤1.25×B；workflow 仅 `push.tags: v*`、`contents: write` 且仅此一项、runner 自带 `gh release create`、零第三方 action；`--check --portable` opt-in；旧格式 bundle 拒绝不归一化；废弃 env（AGATE_REPO_DIR/AGATE_SYMLINK）WARNING 后忽略；fail-closed exit 1；`_protocol_root` 零改动；SHA256SUMS + 确定性打包 P2 可采纳（推荐采纳）。用户另授权：允许新增 `.github/workflows/` release workflow（含触发/权限）。
2. **排除机制必须做真实比对，勿默认**：候选 ≥2（`.gitattributes export-ignore` / 显式清单文件 + 自写打包 / `git archive` + pathspec 白名单 等）。比对维度：默认拒绝语义（S-6）、对其他 archive 消费者影响（GitHub 自动 source archive）、三路径（在线安装/pack-offline/Release 打包）**单一来源**（BDD-3）、可机械验证、Windows 兼容、无 git 环境下的可用性。须用实测（在临时目录对真实仓库跑候选方案）验证，写入 `minimal_validation`。
3. **兼容红线**：`_protocol_root` 函数体不改；已装旧形态可解析；`vdir/scripts` 优先于 `vdir/agate/scripts`。
4. **目录名固定 `agate/`**。
5. **打包逻辑只能有一份**：在线安装、pack-offline、Release workflow 复用同一构建函数/脚本（TAG0031 hash 双实现合并教训）；workflow 内不得再内联一套打包逻辑，只调用仓库脚本。
6. **影响面梳理节（Modify / Not Modify / Risk）必须写在候选方案之前**，落到文件+函数/小节+BDD 编号，证据 grep/rg 命中清单；同类扫描结论承接 P1 §5，不重复劳动但需核验关键命中。
7. **批次设计**：在 frontmatter 声明 `dispatch_plan`（单行 flow YAML；mode/batches/complexity 合法，批数 ≤ parallel_limit）。建议按文件面拆互不冲突批次：A/B/D 核心安装+打包（脚本+测试）、C workflow+Release 构建、E legacy 删除（脚本+测试+文档）、文档面统一；注意 `agate_common.py`/`install.sh`/`UPGRADING.md` 等**共享文件**的批间冲突，串行或明确归属。
8. **gate_commands**（固化后 P4–P6 不可改）：P5 用 CI 口径 `python3 -m pytest agate/tests/ --reruns 1 -n auto`；独立 key：`P5_consistency: "python3 agate/scripts/check-protocol-consistency.py --strict-errors-only"`、`P5_ruff: "~/.venvs/agate-dev/bin/ruff check agate/"`、`P5_shellcheck: "shellcheck -S warning install.sh agate/scripts/*.sh"`；P3 键**只允许裸 `P3`**（用 pytest，勿 `P3_xxx`）；各 key 声明 `_timeout_seconds`（pytest 全量按分片实际耗时，构建类可 600）。`&&` 不得串联多个校验。
9. **前端无**：`ui_affected: false`，无需 UI 设计节。
10. **frontmatter**：`candidate_count`≥2、`packages`、`domains`（含 backend、security）、`ui_affected: false`、`dispatch_plan`；`agent: architect`。用 `agate-md-field-set` 能写的字段走工具，工具拒绝的键（如 agent、dispatch_plan）在返回摘要里说明并手写（主 Agent 已授权你手写这些 frontmatter 键，不得改其他内容）。
11. **CI 验证策略**：判据 5 需推测试 tag 到 origin——设计中给出执行步骤与清理（P5/P6 执行前主 Agent 另行取得用户许可，不在 P2 执行）。
12. **测试策略提示（供 P3）**：BDD-11 红灯回放（修正 `_make_bundle` 前后）、BDD-37 永久 grep 回归白名单、装后目录结构比对（BDD-2/4）、无 git 环境（PATH 剥离 git）。
13. **不改**：hook 三件套、SELF-GATE 机制、`.state.yaml` schema、`rules/*.yaml`、`install-hook.py` 的 `AGATE_HOME` 语义、`resolve_hook_root` 兜底（S-17）。P1 BDD-46 有零 diff 断言。
14. 不写行首 `- PASS`/`- FAIL`。不 git commit。返回：路径 + 一句话摘要（≤30 字）。

### 上游关联

- P1-requirements.md（BDD 权威）、P0-brief.md、HANDOFF-TAG0037.md、roadmap RM-AG0066 详情
- `docs/design-notes/design-rename-execution.md` §8.1
...
- install.sh、agate/scripts/agate-install.py、install-offline.py、agate-pack-offline.py、agate_common.py、agate-resolve.py、resolve-entry.py、agate-summary.py
- agate/UPGRADING.md（版本管理生命周期节）、agate/SETUP.md、agate/tests/README.md
- .github/workflows/protocol-tests.yml（结构参考）、.gitattributes（若存在）
- agate/tests/unit/test_install_offline.py、test_agate_pack_offline.py、regression/test_offline_bundle_roundtrip.py
</dispatch_guide>
