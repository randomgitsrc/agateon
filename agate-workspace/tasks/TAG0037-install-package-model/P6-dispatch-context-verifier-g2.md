...
> 本次是 P6 验收 **第 2 组（BDD-27…52：legacy 删除/软链守卫/文档/平台接入/收口）**（首次派发）。

### 目标

对 P1 的 **BDD-27 至 BDD-52** 逐条验收，产出 `P6-evidence/g2/` 证据与 `P6-evidence/g2/results.md`。

### 组特定要求

- **BDD-42/43/44/45（四平台接入）**：本地实跑 `SETUP.md` 对应节的接入命令（隔离 `AGATE_HOME`/`$AGATE_DIR` 指向隔离版本布局；不得改真实平台配置目录下的文件——若命令会写用户目录如 `~/.claude`/`~/.config/opencode`，改用 pytest 已有的隔离夹具或只做只读探测，并在证据说明）；43/45 缺 CLI 判 FAIL，不跳过；先 `command -v opencode codex claude` 记录可用性。H-1（`claude --agent orchestrator -p "echo test"`，联网/真实凭据）为非 BDD 人工项：**可用则以最小安全 prompt 执行一次并记录，不可用或有风险则明确写"H-1 未执行 + 原因"**，不计入 BDD 总数，也不得凭空写结论。
- **BDD-46（out-of-scope 零 diff）**：按 P3 §7.5 命令 `git diff --stat 75a8102..HEAD -- <清单>` 实跑，留输出（应为空）。
- **BDD-48（pytest 不减反增）**：**全量实跑一次**（CI 口径 `python3 -m pytest agate/tests/ --reruns 1 -n auto`，timeout 580）并留完整汇总；`bash agate/tests/scripts/count-tests.sh`；用例增删对账（P3 §7.5 的 git diff -U0 grep 计数）；skipped 说明。
- **BDD-49（收口）**：`python3 agate/scripts/check-protocol-consistency.py --strict-errors-only`（worktree 自己的脚本）、`~/.venvs/agate-dev/bin/ruff check agate/`、`shellcheck -S warning install.sh agate/scripts/*.sh` 各自实跑并留输出；④ 由 `test_doc_sweep.py` 相关用例证明。
- **BDD-50（发布物）**：此 BDD 的 ②–⑥ 属 P8/发布时事实（版本号 bump、CHANGELOG `[Unreleased]`→`[0.73.0]`、tag、roadmap 回写、HANDOFF 归档、正式 Release 资产、PR 描述），P6 阶段**不可能已发生**。主 Agent 决策：P1 BDD-50 将由主 Agent 授权追加 `[BASELINE_CHANGE]` 注记，明确"BDD-50 在 P6 的验收范围 = 发布前就绪度：`agate/UPGRADING.md` 存在 `### v0.73.0` 节且标注 BREAKING 与迁移三步、`CHANGELOG.md [Unreleased]` 已含本任务条目与 BREAKING 标注及 UPGRADING 指针、README badge 不为 1.0、release workflow 的触发/权限可如实列出（引用 release.yml 静态事实）；②–⑥ 的最终事实在 P8 复验并写入 P8-release.md"。你据此判定：就绪度各项证据（读文件内容 + 命令输出）满足即写 PASS，并在 PASS 描述里写明"P6 验收范围=发布前就绪度（BASELINE_CHANGE），最终事实 P8 复验"；任一就绪度项不满足则 FAIL。
- 其余 BDD（27–41、47、51、52）：pytest 映射测试实跑（`P3-test-cases.md` §8.1）+ BDD 文字要求的命令面证据（如 BDD-31/32/33 软链守卫 5 种变体 `L L/ L// L/. L/..` 的真实调用退出码与目标目录不变；BDD-37 全仓 grep 回归实扫命中数=23 且落在白名单内；BDD-36 迁移三步可走通）。
