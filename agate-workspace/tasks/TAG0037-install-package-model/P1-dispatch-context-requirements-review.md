...
独立评审 `P1-requirements.md`（50 条 BDD，子批 A–E，映射 P0-brief 11 条完成判据），产出 `P1-review.md`（frontmatter `status: approved | needs-revision`，`agent: requirements-review`，正文含 BDD 编号引用与覆盖维度标注）。只审不写，不改 P1-requirements.md。

### 约束

1. 按角色文件检查清单逐项审；重点：BDD 二值可判定、编号连续（`#### BDD-NN:`）、单 Given-When-Then、跨条一致性、隐含需求（数据/边界/兼容）。
2. **P0-brief 11 条判据逐条核对有 BDD 覆盖**（表格：判据 → BDD 编号），缺覆盖即 needs-revision。
3. **兼容红线**：`_protocol_root` 探测序不得改语义；已装旧形态 `vX.Y.Z/{agate,docs,...}` 必须仍可解析——审是否有独立 BDD 且可判定。
4. **范围纪律**：是否有超出 P0-brief 的内容（应标 `[SCOPE+]` 而非直接写进 BDD）；用户已裁决三项（CI 许可含触发/权限；legacy 仅文档三步 + fail-closed 提示、无迁移工具；minor bump，最新 tag 为 v0.72.0 故本任务 v0.73.0）已按此写入，勿再当未决项。
5. **`[SUGGEST]` 项（S-1…S-19）**审其理由是否成立、是否有 SUGGEST 实为需人定夺的业务方向/破坏性变更（应升级为 NEED_CONFIRM）；重点看 S-1（tests 排除）、S-3/S-4（LICENSE/NOTICES/CHANGELOG 入包，精确化"唯一内容=agate/"）、S-13（旧格式 bundle 拒绝而非归一化）、S-15（exit 1）。
6. **同类扫描节**：核对命中数与清单是否真实（可自行 grep 抽查 `use_legacy` / `_protocol_root` 等），逐条有处理判定。
7. **测试/验证可行性**：判据 5（测试 tag 实跑，须用户当次许可、清理方式）、判据 6（无 git 环境）、判据 7（冗余阈值）BDD 是否真能被验证。
8. **P0_STALE 三条**（v0.72.0 已发布 / `agate/CLAUDE.md` 不存在 / 测试文件路径）：抽查其真实性。
9. 评审意见中不使用行首 `- PASS` / `- FAIL` 格式。发现须分级（BLOCKER/MAJOR/MINOR），给出可执行修改建议。

### 上游关联

- P0-brief.md、HANDOFF-TAG0037.md、agate-workspace/roadmap/roadmap.md（RM-AG0066 详情）
...
- agate/scripts/agate_common.py、agate/scripts/agate-install.py、install.sh、agate/scripts/install-offline.py、agate/scripts/agate-pack-offline.py（抽查事实性论断）
</dispatch_guide>
