...
> 你是本次 P2 评审的**专家组组长**（角色：review，汇总），不是新的评审员。

### 目标

汇总 `P2-review-eng.md`（plan-eng-review）与 `P2-review-cso.md`（cso）两份评审，产出 `P2-review.md`（统一 `status: approved | rejected`，`agent: review`）。

### 约束

1. **不发表新意见，只汇总**：列两位专家最终结论、遗留发现（含 MINOR/MEDIUM/LOW）及其在 P2-design.md 的处置状态。
2. 规则：任一专家标 BLOCKER（HIGH 及以上视为 BLOCKER）→ rejected；专家分歧 → 标"专家组分歧"；全票无 BLOCKER → approved。两份当前均为 approved 状态，请核对文件 frontmatter `status` 真实值，不凭本说明。
3. 核对首轮 BLOCKER/HIGH/MAJOR/MEDIUM 均已在设计中处置（读 P2-design.md §14 处置表抽查 3 条以上）；两位专家复审后提出的非阻塞项（cso N-1/N-2、eng N-1…N-5）主 Agent 已让 architect 增补进 §14.4，抽查其真实存在（不真实存在则如实标出并 rejected）。
4. P2-review.md 的 Header `agent` 必须是 `review`（非 main）；正文含 BDD 编号引用锚点（如覆盖的关键 BDD：BDD-4/9/11/16/27/28/32/51/13/20）；不用行首 `- PASS`/`- FAIL`。
5. 数据安全：只读；除输出文件外不改任何仓库文件；不 git commit；不删除任何文件。
6. 返回：路径 + 一句话摘要（≤30 字）。
