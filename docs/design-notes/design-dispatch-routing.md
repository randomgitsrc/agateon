...
6. **RM 编号：已申领 RM-AG0060**（`agate-workspace/roadmap/roadmap.md`，status backlog，epic）。排期——检查是否与近期涉及 `dispatch-protocol.md` / 推进侧 CLI 的任务冲突（尤其 **RM-AG0059 任务管理命令化** 有 CLI 家族重叠，backlog）。**epic 拆分倾向**：a 配置路由核心 + `cli: native`（Claude Code/OpenCode）/ b 跨 CLI 子进程 + Codex 首次接入（`platform-notes.md` Codex 章 + `CodexAdapter` + SETUP）/ c tmux 观测层（可后置/可选，不通过可整体移除不影响 a/b）。参照 RM-AG0058 epic 形态。
