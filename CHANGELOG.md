...
- **`--separate-git-dir` / submodule 不再被误判为"链接 worktree"**：这两类布局下 `.git` 也是文件、`git worktree list` 首行给的是 **gitdir**（如 `<repo>/.git`、`<sup>/.git/modules/...`）。原先会把它当宿主项目登记进台账（`--list` 显示 git 内部目录），并打印错误的"对本仓库所有 worktree 生效"提示。现宿主判定要求**确实是工作树**（其下存在 `.git`）。
