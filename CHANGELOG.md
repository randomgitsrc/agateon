...
- **DSH preset 接入长期「假成功」**（DSH ≥ **0.1.7-alpha.1** 起，上游 commit `d1e22a7e24`）：DSH 已**不再读取**目录式 preset `$DSH_HOME/.agent-presets/<id>/`（上游 skill 原文："Nothing reads that directory any more."），而 `agate-setup.py` 仍往那个目录写、`--list` 仍检查它 → 工具一路报 ✅ 而 DSH 里根本没有该模式。**实测代价**：本机最后一个用上 agate preset 的会话停在 2026-09-20 22:25，其后每个会话都落 `standard`。现改为往 **profile 自己的 patch 文件** `~/.dsh/profiles/*/cordis.patch.yml` 写**声明块**（`- insert:` 行：`id: preset-agate` + 插件 `@deepseek-ai/dsh-agent-preset` + `config.id: agate`，外加 `name`/`description`/`order` 与 `config.plugins`），内容由 `assets/templates/dsh/{preset.yml,agent.cordis.yml}` 两个模板生成（模板仍是唯一来源；生成时自动把 0.1.7 已删除的 `@deepseek-ai/dsh-workflow-worker-thread` 换成 `@deepseek-ai/dsh-workflow-ptc`——照抄旧包名会**激活失败**）。块以定界符托管：幂等写入、卸载只摘两标记之间的内容；`--list` 与 `agate-summary.py` 改查 **DSH 真正读取的位置**，`--uninstall` 精确摘除该块并顺带清掉旧死目录（只删指向本安装的软链，用户自己的文件保留）。**已接入用户重跑一次 `python3 ~/.agate/scripts/agate-setup.py` 即完成迁移**；旧的 `.agent-presets/agate/` 可直接删除。
...
- **平台接入产物漂移检测在部分调用路径下失效或误报**（v0.73.0 起存在）：原实现只以「运行脚本所在树」上溯定权威根，导致三条调用路径里只有一条正确——① 版本目录直接调用正常；② `~/.agate/scripts/`（**SETUP.md 规定的调用方式**，根级副本）上溯到 `~/.agate` 而非协议根 → 模板找不到 → **检测静默失效、真漂移漏报**；③ 从开发 checkout / worktree 调用 → 运行树是开发态而产物指向安装态 → **误报漂移**。现改为「权威 = 任一**已安装版本**树内的同名模板」：指向 dev 树 / 临时副本仍被报出（原始缺陷类），而项目 `.agate-version` 钉版**不再**引起误报与双向振荡。
- **未安装提示聚合**：原先每平台一行（本机四平台目录齐备时一次输出 3 行），现聚合为一行且措辞覆盖「未安装或不完整」；漂移/过期仍逐条报。
- **产物检测改为两级信号**：① **漂移**（警告）——指向**不在任何已装版本树内**的目标（异物 / 临时副本 / dev checkout；权威集只认 `vX.Y.Z` 版本目录，不含 `repo/` clone 与根 `scripts/` 副本）；② **版本落后**（信息级）——指向**已装但非机器 `current`** 的版本。后者是升级后的常见状态（装新版不删旧版，且产物指向具体版本目录），此前静默会让人误以为在用新版；判据用机器 `current`（项目无关），故项目 `.agate-version` 钉版不会引起误报。
- **移除 `os.name == "nt"` 整体跳过**（改为"平台目录不存在即跳过"）——Codex 在 Windows 亦会安装，不应被跳过；相关测试同步去掉 3 处 Windows 跳过（改用「建链失败即 skip」的平台无关写法）。
- **`agate-debt-check.py` 补 list 元素类型校验 + 拒绝空证据**：此前只校验 `evidence` / `closure_criteria`「是 list」，实测可放过**挂错清单**的条目（如 closure_criteria 里混进 `{path, note}` 证据）。现要求 evidence 元素为映射、closure_criteria 元素为字符串；并**拒绝空/无已知键的 evidence 条目**（`evidence: [{}]` 原可 rc=0 放行）。存量 49 条 / 183 个 evidence dict 已全量扫描，**零误伤**。
...
- **`agate-summary.py` 的平台接入产物漂移检测扩至四个平台**：原仅覆盖 DSH，现覆盖 Claude Code / OpenCode / DSH / Codex 的全局接入产物（与 `agate-setup.py` 的 `PLATFORMS` 表机械核对，防漏登记）。判定改为**按形态分判**——软链比对真实路径、**复制形态比对内容**：正常复制安装不再被误报为「漂移」；§下条「两级信号」描述了复制形态的两种告警条件（内容 = 某个已装版本 → 「落后」；与所有已装版本都不一致 → 「已过期」）。
