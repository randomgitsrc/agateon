...
- **`agate-setup.py --uninstall`（完整卸载）**：此前只有"装"没有"卸"——文档让用户 `rm -rf ~/.agate`，但那**只删本体**，平台接入物与项目侧 git hook 会变成**断链**（断链的 hook 让 `git commit` 直接失败，实测）。现提供对称卸载：`--list` 查看、`--dry-run` 预览、`--uninstall` 清接入物（`--scope global|project`）、`--all-projects` 清台账里每个项目、`--purge` 删本体。
