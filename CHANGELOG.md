...
- **`agate-setup.py --uninstall`（完整卸载）**：此前只有"装"没有"卸"——文档让用户 `rm -rf ~/.agate`，但那**只删本体**，平台接入物与项目侧 git hook 会变成**断链**（**复制模式下的陈旧 hook 会让 `git commit` 直接失败**；软链模式的断链会被 git 静默忽略——2026-09-21 实测修正）。现提供对称卸载：`--list` 查看、`--dry-run` 预览、`--uninstall` 清接入物（`--scope global|project`）、`--all-projects` 清台账里每个项目、`--purge` 删本体。
  ...
  - **`--purge` 守卫**：目标不像 agate 安装根（缺 `scripts/agate-install.py`）时 fail-closed，防 `AGATE_HOME` 指错而误删无关目录。
