...
- **`agate-debt-check.py` 补 list 元素类型校验**：此前只校验 `evidence` / `closure_criteria`「是 list」，实测可放过**挂错清单**的条目（如 closure_criteria 里混进 `{path, note}` 证据）。现要求 evidence 元素为 `{path/ref, note}` 映射、closure_criteria 元素为字符串（存量 49 条已全量扫描，无误伤）。
