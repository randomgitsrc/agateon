...
**2026-09-11 记录（post-09 a-new-session-is-not-a-new-perspective，模型多样性篇，配 v0.71.0 发布位）**：A1-9 ✓（写稿前事实批：局限 2/ADR-006 原文含改造前"无解"历史口径 git show 核、§0 五步+两不变量原文、DISPATCH_ROUTE_REASONS 三值枚举、BDD-40 零变更测试、本仓 routing 空配置诚实点、两轮外部评审 FAIL→PASS 复述、v0.71.0 tag）10-11 ✓（**vision-engine 后端故障：OCR deferred，SVG 源核验 + 像素 sanity + 几何 bbox 粗检兜底，评审 subagent 按替代口径核 SVG；发布前后端恢复，4 图 OCR 补齐全过**）12 ✓（评审 2 轮：FAIL 警句 ≥9 超配额 → 修 10 处 → PASS；非阻塞 anaphora 合并顺手落）13-14 ✓（双语 200，站点日期 20260911）15 当日未 cross-post 过 → 可发 16 ✓（dev.to id 4630568，canonical→agateon.com，公开 200）17-18 待手动。**坑记录**：zh frontmatter description 内 ASCII 双引号截断 YAML（parseMatter 报错）→ 换全角引号；插图 4.7KB 超 Vite 内联阈值出 assets 文件（先例 post-07，文件名 grep 字符类记得含 `-`）。

**2026-09-19 记录（post-10 should-your-agent-workflow-use-a-protocol，决策框架篇）**：A1-9 ✓（写稿前事实批：算分源码五信号/tier 合成、真实算分输出、check-routing 单向性原文 L19/L21、thin 四要素、成本数字全量重跑、账本 exit 语义按 phases.yaml gate_pass_exit 重算）10-11 ✓（双图 OCR+布局质检；zh SVG 双重转义 `&amp;#183;`/`&amp;#8594;` 已修重渲染）12 ✓（**评审 3 轮**：首轮 FAIL 5 处必改——exit 2 被写成"不适用"（实为 P0-P3/P5/P6/P8 的正常通过码，与 post-07 口径冲突）、工期 35h 口径与分母矛盾、账本分母 37→14、代码块标称 real 却含英译、单向性"never less"过度概括；二轮 FAIL 4 处**新引入**——35h 子集口径建立在 git 迁移断层（54fdb02）上属把 artifact 当事实、description 仍旧口径且进 meta、zh SVG 实体双重转义、第一人称归属"我们"→"我"；三轮 PASS 含 2 处非阻塞顺手修）13-14 ✓（双语 200，站点日期 20260919）15 当日未 cross-post 过 → 可发 16 ✓（dev.to id 4692755，canonical→agateon.com，公开 200）17-18 待手动。**核心教训**：主张"数字必须对源"的文章，评审价值全在数字——本轮评审抓出的 5+4 处里，最严重的是作者自己算错/口径混淆的数，且其中一处（35h）源自"采纳评审上轮建议但没核根因"，说明建议也要验证而非照抄。

**2026-09-19 补记（post-10 二轮重写——读者视角修正）**：用户反馈①"blog 围绕自身 agate-workspace 数据、目标/建议以 Agateon 为坐标，别的项目不关心你们怎么 dogfooding，只关心用 Agateon 能不能带来收益" → 诊断：把**素材来源**（我们的数据）当成了**主题**（读者收益）；dogfooding 数据是好论据不是好主题。修复①：BLOG-STANDARDS 新增 §1.1 读者收益门槛（选题硬门槛：读者能拿走什么/脱离 Agateon 是否成立/自指数据配额）+ §7 评审项 + §8 自检项（PR #348）。用户反馈②（对首轮重写）："加'你到底得到什么'节太模式化、欲盖弥彰，读者看完自有判断，整篇要考虑受众" → 修复②：删显式收益节，收益融回论证（hook 交易框架/静默失败 vs 崩溃/评审位置/注意力是价格），推销腔清零（you can steal / the payoff 等），章节名回归陈述式。最终评审 PASS：主题归位（读者的决策）、收益自然浮现于论证、自指数据仅剩成本校准服务论点、事实全对源。**方法教训**：①"被指出读者视角缺失"的**错误反应**是加一节"收益声明"——那正是营销体；真为读者写的文章不需要 declare 收益 ②收益必须长在论证里（hook 立交易、正文证代价、结尾给可搬走的东西），显式 declare = 正文没长在读者位置上 ③`"不推销" ≠ "没价值"`：删节不能删到只剩机制，静默失败/评审位置/注意力价格这些点必须还在（只是不说破）。
